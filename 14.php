...
<style>
   ...
   .accessory-selection-container.has-selection {
      border-color: #4361ee;
      background: linear-gradient(135deg, #4361ee08 0%, #3a0ca308 100%);
   }

   .accessory-selection-container.has-selection .accessory-option-preview {
      background: linear-gradient(135deg, #4361ee08 0%, #3a0ca308 100%);
      border-left-color: #4361ee33;
   }
   ...
   .accessory-selection-container:hover .accessory-option-preview {
      background: #f1f3ff;
   }
                     ...
                     <div class="qualification-modal" id="mainCategoryModal">
                        <div class="qualification-modal-content">
                           ...
                           <div class="qualification-modal-footer">
                              ...
                              <button type="button" class="btn btn-primary" id="confirmMainCategory" disabled>Next</button>
                           </div>
                        </div>
                     </div>
                     ...
                     <div class="multi-select-modal" id="multiSelectModal">
                        <div class="multi-select-modal-content">
                           ...
                           <div class="multi-select-modal-footer">
                              <button type="button" class="btn btn-secondary" id="closeMultiSelectModal">Close</button>
                              <button type="button" class="btn btn-secondary" id="multiSelectModalBackButton">Back</button>
...
<script>
   jQuery(function($) {
      ...
      function createMaterialInputsForCategory(product, variant, category, roomId, activeMaterials) {
         ...
         if (categoryData.materialGroups && Object.keys(categoryData.materialGroups).length > 0) {
            return Object.entries(categoryData.materialGroups).map(([refLabel, materials]) =>
               createMaterialGroupInputs(product, variant, category, refLabel, materials, roomId, variantId)
            ).join('');
         } else {
            // Fallback to single material input
            return createSingleMaterialInputs(product, variant, category, roomId, variantId);
         }
      }

      // Function to create material inputs for a specific material group (A, B, C, etc.)
      function createMaterialGroupInputs(product, variant, category, refLabel, materials, roomId, variantId) {
         return `
            <div class="material-group" data-ref-label="${refLabel}">
                  <div class="material-group-header">
                     <h6>Material ${refLabel}</h6>
                  </div>
                     ...
                     <div class="material-compact-fields">
                        <div class="material-input">
                              ...
                              <select class="form-control material-grade" 
                                    data-variant="${variantId}" data-category="${category.id}" data-ref-label="${refLabel}">
      ...
      function createSingleMaterialInputs(product, variant, category, roomId, variantId) {
         return `
            <div class="material-inputs-compact">
                  <div class="material-compact-image">
                     <i class="fa fa-image"></i>
                  ...
                  <div class="material-compact-fields">
                     <div class="material-input">
                        <label>Material Grade</label>
                        <select class="form-control material-grade" 
                              data-variant="${variantId}" data-category="${category.id}">
                              <option value="">Select Grade</option>
                              <option value="standard">Standard</option>
                              <option value="premium">Premium</option>
                              <option value="economy">Economy</option>
      ...
      function createPillowSubcategoriesForVariant(product, variant, roomId, activeMaterials) {
         ...
         if (product.material_categories && Array.isArray(product.material_categories)) {
            materialCategories = product.material_categories;
         } else if (product.product_active_materials && typeof product.product_active_materials === 'string') {
            try {
               const activeMaterialIds = JSON.parse(product.product_active_materials);
               materialCategories = materialCategories.filter(cat => activeMaterialIds.includes(cat.id));
            } catch (e) {
               materialCategories = [];
            }
         }
         ...
         const hasMaterialGroups = pillowData.materialGroups && Object.keys(pillowData.materialGroups).length > 0;
                ...
                ${pillowCategory.subcategories.map((subcat, index) => `
                    <div class="pillow-subcategory-content ${index === 0 ? 'active' : ''}" 
                       ...
                       <div class="pillow-subcategory-details">
                           ${hasMaterialGroups ? 
                               createPillowMaterialGroups(product, variant, subcat, pillowData, roomId, variantId) :
                               createSinglePillowMaterialInputs(product, variant, subcat, pillowCategory, roomId, variantId)
                           }
                       </div>
                    </div>
                `).join('')}
    ...
    `;
      ...
      // Function to create pillow material groups (A, B, C, etc.)
      function createPillowMaterialGroups(product, variant, subcat, pillowData, roomId, variantId) {
         const materialGroups = pillowData.materialGroups || {};

         return Object.entries(materialGroups).map(([refLabel, groupData]) => {
            const subcatMaterials = groupData[subcat.id] || [];
            const availableMaterials = pillowData[subcat.id]?.all_materials || pillowCategory.defaultMaterials || [];

            return `
            <div class="pillow-material-group" data-ref-label="${refLabel}">
                <div class="pillow-material-group-header">
                    <h6>Pillow ${subcat.name} - ${refLabel}</h6>
                </div>
      ...
      // Function for single pillow material input (fallback)
      function createSinglePillowMaterialInputs(product, variant, subcat, pillowCategory, roomId, variantId) {
         return `
        <div class="pillow-material-inputs-compact">
            <div class="pillow-material-compact-image">
                <i class="fa fa-image"></i>
    ...
    `;
      ...
      function createCurtainVariantContent(product, variant, roomId) {
         ...
         const price = dims.standart_price || variant.standart_price || '';
         ...
         return `
            <div class="variant-details">
               ...
               <!-- Material Section for Curtain Variant -->
               <div class="material-section">
                  <h6><i class="fa fa-layer-group mr-2"></i>Material Selection for ${variant.product_name}</h6>
                  <div class="material-tabs" id="materialTabs-${product.product_id}-${variant.product_id}-room${roomId}">
                  ...
                  <div class="material-tabs-content" id="materialTabsContent-${product.product_id}-${variant.product_id}-room${roomId}">
                     ${materialCategories.map((category, index) => `
                        <div class="material-tab-content ${index === 0 ? 'active' : ''}" 
                           id="materialContent-${product.product_id}-${variant.product_id}-room${roomId}-${category.id}">
                           ${category === 'pillow' ? 
                              createPillowSubcategoriesForVariant(product, variant, roomId) : 
                              createStandardMaterialContent(product, variant, category, roomId)
                           }
                        </div>
                     `).join('')}
                  </div>
                     ...
                     <div class="accessory-layout">
                        <div class="accessory-tabs-sidebar">
                           <div class="accessory-tabs-header">
                              ...
                              <button type="button" class="btn btn-sm btn-primary add-accessory-btn" data-product="${product.product_id}" data-type="${product.type}" data-available-in="${product.available_in}" data-variant="${variant.product_id}" data-room="${roomId}">
         ...
         $.post(ajax_url + '/api', {
            ...
            if (response.status === 'success') {
               ...
               variants.forEach(variant => {
                  ...
                  // Setup accessory add button
                  $(`#variant-${productId}-${variant.product_id}-room${roomId} .add-accessory-btn`).on('click', function() {
                     const productId = $(this).data('product');
                     const variantId = $(this).data('variant');
                     const roomId = $(this).data('room');

                     console.log('Add accessory to curtain variant:', {
                        productId,
                        variantId,
                        roomId
                     });
                     showAccessorySelectionModal(productId, variantId, roomId);
                  });
               });
            }
         });
      ...
      $(document).on('click', '.simple-product-content .add-accessory-btn', function() {
         const productId = $(this).data('product');
         const variantId = $(this).data('variant');
         const roomId = $(this).data('room');
      ...
      function createVariantContentForSize(product, variant, roomId) {
         return `
            <div class="variant-details">
                  <div class="compact-product-details">
                     <div class="compact-section-header">
                        <h6><i class="fa fa-cube mr-2"></i>${product.product_name} - ${product.product_desc}</h6>
                     </div>
                     <div class="compact-details-with-image">
                        <div class="compact-image-preview">
                              <img style="width:100%;height:100%;" src="${product.product_img}" alt="${product.product_name}">
      ...
      // Updated function to setup variant material tabs
      ...
      function setupMaterialTabsForProduct(product, roomId) {
         const materialTabsId = `materialTabs-${product.product_id}-room${roomId}`;
         ...
         setTimeout(() => {
            if ($(`#${materialTabsId}`).length) {
               $(`#${materialTabsId} .material-tab`).off('click').on('click', function(e) {
                  e.preventDefault();
                  const categoryId = $(this).data('category');
                  const materialTabsContentId = `materialTabsContent-${product.product_id}-room${roomId}`;

                  $(`#${materialTabsId} .material-tab`).removeClass('active');
                  $(this).addClass('active');

                  $(`#${materialTabsContentId} .material-tab-content`).removeClass('active');
                  $(`#materialContent-${product.product_id}-room${roomId}-${categoryId}`).addClass('active');
               });
            }
         }, 100);
      }
            ...
            try {
               ...
               if (response.status === 'success') {
                  ...
                  mainCategories.forEach((category, index) => {
                     ...
                     const randomImage = `https://picsum.photos/200/200?random=category${category.category}`;

                     const $option = $(`
                        <div class="qualification-option" data-category="${category.id}">
                            <div class="qualification-select-option-image">
                                <img src="${randomImage}" alt="${category.category}" onerror="this.src='https://picsum.photos/200/200?random=${category.category}'">
                            </div>
         ...
         $.post(ajax_url + '/api', {
            ...
            if (response.status === 'success') {
               ...
               filteredQualifications.forEach((qual, index) => {
                  const $option = $(`
               <div class="qualification-option" data-qualification="${qual.attr_ids}">
                  <div class="qualification-select-option-image">
      ...
      function initializeMultiSelectModal(qualifications, roomId) {
            ...
            if (response.status === 'success') {
               ...
               styles.forEach(style => {
                  ...
                  $styleTab.on('click', function() {
                     ...
                     const qualifications = $('#multiSelectModal').data('qualification');
                     applyProductFilters(qualifications);
                  });
      ...
      function showMultiSelectModal(qualifications, roomId) {
         ...
         initializeMultiSelectModal(qualifications, roomId);
    ...
    `);
      ...
      function showAccessorySelectionModal(productId, variantId, roomId) {
      ...
      function addAccessoryToCurtainVariant(roomId, productId, variantId, accessory) {
         ...
         if (product.has_variant && product.available_in === 'set') {
            ...
            try {
               ...
               variants = Object.entries(bedDimsObject).map(([key, value]) => ({
                  ...
                  name: `${key} (${value.width || ''}x${value.length || ''})`,
            ...
            if (product.type === 'fitout') {
               ...
               loadCurtainProductContent($content, product, roomId);
            } else {
               loadSimpleProductContent($content, product, roomId);
            }

            // Setup material tabs
            setupMaterialTabs(product.product_id);

            // Setup pillow subcategories if needed
            if (product.type !== 'fitout') {
               setupPillowSubcategoryTabs(product, roomId);
            }
      ...
      function loadSimpleProductContent($content, product, roomId) {
         ...
         const length = dims.length || '';
         const height = dims.height || '';
         const price = dims.standart_price || product.standart_price || '';
         ...
         const $wrapper = $(`
            <div class="simple-product-content">
                  <div class="compact-product-details">
                     ...
                     <div class="compact-details-with-image">
                        ...
                        <div class="compact-details-fields">
                              ...
                              <div class="compact-detail-group">
                                 <label>Height (m)</label>
                                 <input type="number" class="form-control dimension-height" value="${height}" placeholder="0.00" step="0.01" min="0">
                              </div>
                              <div class="compact-detail-group">
                                 <label>Quantity</label>
                                 <input type="number" class="form-control product-qty" 
                  ...
                  <!-- Material Section - Use existing data -->
                  ${createMaterialSection(product, mockVariant, roomId)}
                  </div>
            </div>
         `);
         ...
         setTimeout(() => {
            setupMaterialTabsForProduct(product, roomId);
         }, 100);
      }

      // Curtain product content with accessory section working like items section
      function loadCurtainProductContent($content, product, roomId) {
         let materialCategories = [];
         ...
         const $wrapper = $(`
            <div class="simple-product-content">
               ...
               <div class="material-section">
                  ...
                  <div class="material-tabs" id="materialTabs-${product.product_id}">
                     ${materialCategories.map((category, index) => `
                     <button class="material-tab ${index === 0 ? 'active' : ''}" 
                           data-category="${category}">
                        ${category.charAt(0).toUpperCase() + category.slice(1)}
                  ...
                  <div class="material-tabs-content" id="materialTabsContent-${product.product_id}">
                     ${materialCategories.map((category, index) => `
                        <div class="material-tab-content ${index === 0 ? 'active' : ''}" id="materialContent-${product.product_id}-${category.id}">
                           ${category === 'pillow' ? 
                              loadPillowSubcategories(product) : 
               ...
               <div class="curtain-options-section">
                  ...
                  <div class="accessory-layout">
                     <div class="accessory-tabs-sidebar">
                        <div class="accessory-tabs-header">
                           <h6><i class="fa fa-list mr-2"></i>Accessories</h6>
                           <button type="button" class="btn btn-sm btn-primary add-accessory-btn" data-product="${product.product_id}" data-variant="" data-room="${roomId}">
                              <i class="fa fa-plus mr-1"></i> Add
                           </button>
                        </div>
      ...
      function loadPillowSubcategories(product) {
         const productId = product.product_id;
         let materialCategories = [];
      ...
      function createMaterialSection(product, variant, roomId) {
         ...
         const materialCategories = Object.keys(activeMaterials).map(category => {
            if (category === 'pillow') {
               return {
                  id: category,
                  ...
                  subcategories: [{
                        id: 'default_pillow',
                        name: 'Default Pillow',
                        all_materials: activeMaterials.pillow.default_pillow.all_materials || []
                     },
                     {
                        id: 'pillow_front',
                        name: 'Pillow Front',
                        all_materials: activeMaterials.pillow.pillow_front.all_materials || []
                     },
                     {
                        id: 'pillow_back',
                        name: 'Pillow Back',
                        all_materials: activeMaterials.pillow.pillow_back.all_materials || []
                     },
                     {
                        id: 'pillow_pipping',
                        name: 'Pillow Pipping',
                        all_materials: activeMaterials.pillow.pillow_pipping.all_materials || []
                     }
                  ],
                  active_materials: activeMaterials.pillow
               };
            } else {
               return {
                  id: category,
                  name: category.charAt(0).toUpperCase() + category.slice(1),
                  all_materials: activeMaterials[category].all_materials || [],
                  active_materials: activeMaterials[category].active || []
               };
            }
         }).filter(category => category.all_materials.length > 0 || category.active_materials.length > 0);
                     ...
                     ${materialCategories.map((category, index) => `
                        <div class="material-tab-content ${index === 0 ? 'active' : ''}" 
                           ...
                           ${category.id === 'pillow' ? 
                                 ...
                                 createStandardMaterialContent(product, variant, category, roomId, activeMaterials)
                           }
                        </div>
                     `).join('')}
               </div>
               ...
               <div class="material-tabs-content" id="materialTabsContent-${itemId}-${productId}-room${roomId}">
                  ${materialCategories.map((category, index) => `
                     <div class="material-tab-content ${index === 0 ? 'active' : ''}" id="materialContent-${itemId}-${productId}-room${roomId}-${category.id}">
                        ${category.id === 'pillow' ? 
                           loadPillowSubcategoriesForItem(item, productId, roomId) : 
      ...
      function setupPillowSubcategoryTabs(product, roomId) {
         let productId = product.parent_id;
         ...
         $pillowTabs.find('.pillow-subcategory-tab').on('click', function(e) {
            ...
            updatePillowSubcategoryStatus(productId, productId, availableIn, subcategoryId, roomId, product.type);
         });
      ...
      function addItemToProduct(roomId, productId, item) {
         ...
         const dims = item.dims || {}; // fallback if undefined
               ...
               <div class="enhanced-category-item">
                  ...
                  <div class="enhanced-details-with-image">
                     ...
                     <div class="enhanced-details-fields">
                        <div class="detail-group">
                           <label>Length (m)</label>
                           <input type="number" class="form-control item-length item-dims" value="${length}" placeholder="0.00" step="0.01" min="0">
                        </div>
                        <div class="detail-group">
                           <label>Width (m)</label>
                           <input type="number" class="form-control item-width item-dims" value="${width}" placeholder="0.00" step="0.01" min="0">
                        </div>
                        <div class="detail-group">
                           <label>Height (m)</label>
                           <input type="number" class="form-control item-height item-dims" value="${height}" placeholder="0.00" step="0.01" min="0">
                        </div>
                        <div class="detail-group">
                           <label>Quantity</label>
      ...
      $(document).on('click', '#multiSelectModalBackButton', function() {
      ...
      $('#confirmAddQualification').on('click', function() {
         ...
         if (state.selectedQualification && state.currentRoom) {
            ...
            setTimeout(() => {
               ...
               showMultiSelectModal(state.selectedQualification, state.currentRoom);
            }, 100);
         }
      });
      ...
      // Also fix the confirmSelectAccessory handler to get the correct context
      $('#confirmSelectAccessory').on('click', function() {
         ...
         if (state.selectedAccessory) {
            ...
            if (state.currentVariantId && state.currentProductId && state.currentRoomId) {
               ...
               addAccessoryToCurtainVariant(state.currentRoomId, state.currentProductId, state.currentVariantId, state.selectedAccessory);
