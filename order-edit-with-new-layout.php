                    ...
                    <div class="card-body">
                        <form id="edit_order_form" enctype="multipart/form-data">
                            <div class="row">
                                ...
                                <div class="order-page-col col-md-3 col-sm-3 full-w">
                                ...
                                <div class="order-page-col col-md-6 col-sm-6 full-w-s">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea name="order_agreement_text" id="lblOrderAgreementText" rows="10" class="form-control order-agreement-text required" placeholder="<?= get_lang_text('orderadd_input_contract_text'); ?> *"><?= str_replace('<br />', '', html_ent_decode($order->agreement_text)); ?></textarea>
                                ...
                                <div class="order-page-col col-md-6 col-sm-6 full-w-s">
                                    ...
                                    <div class="form-group row show-extra-agreement-row d-none" style="display: <?= $order->agreement_extra == '1' ? 'flex' : 'none'; ?>;">
...
<script>
    ...
    function createMaterialSection(product, variant, roomId, existingData, productInstanceId, setIndex = 0) {
            ...
            <div class="material-tabs-content" id="materialTabsContent-${productInstanceId}-${variantId}-room${roomId}">
                ${materialCategories.map((category, index) => `
                    <div class="material-tab-content ${index === 0 ? 'active' : ''}" 
                       ...
                       ${createMaterialContentWithLabels(product, variant, category, roomId, activeMaterials, productInstanceId, existingData, setIndex)}
                    </div>
                `).join('')}
            </div>
        </div>
    ...
    function createMaterialContentWithLabels(product, variant, category, roomId, activeMaterials, productInstanceId, existingData, setIndex) {
        ...
        if (category.id === 'pillow') {
            return createPillowStructure(product, variant, categoryData, roomId, productInstanceId, variantId, existingData, setIndex);
        }
            ...
            <div class="material-label-content" id="${category.id}LabelContent-${productInstanceId}-${variantId}-room${roomId}">
                ${labelTabs.map((labelTab, index) => `
                    <div class="material-label-tab-content ${index === 0 ? 'active' : ''}" 
                       ...
                       ${createMaterialLabelContent(product, variant, category.id, labelTab.label, categoryData, roomId, productInstanceId, variantId, existingData, setIndex, index)}
                    </div>
                `).join('')}
            </div>
        </div>
    ...
    function createPillowStructure(product, variant, pillowData, roomId, productInstanceId, variantId, existingData, setIndex) {
                        ...
                        <div class="pillow-subcategories-content" id="pillowSubContent-${productInstanceId}-${variantId}-room${roomId}-${labelTab.label}">
                            ...
                            <div class="pillow-subcategory-content ${subIndex === 0 ? 'active' : ''}"
                                ...
                                <div class="pillow-subcategory-details">
                                    ${createPillowSubcategoryContent(product, variant, subcat, labelTab.label, pillowData, roomId, productInstanceId, variantId, existingData, setIndex, index, subIndex)}
                                </div>
                            </div>
                            `).join('')}
                        </div>
    ...
    function createPillowSubcategoryContent(product, variant, subcat, label, pillowData, roomId, productInstanceId, variantId, existingData, setIndex, labelIndex, subIndex) {
        ...
        // Check for existing material data - FIXED: Don't redeclare the variable
        if (existingData?.materials?.main?.[setIndex + 1]?.[category]) {
            // Use the existing array and find the specific label index
            const categoryMaterials = existingData.materials.main[setIndex + 1][category];
            console.log(`categoryMaterials:${setIndex}`, categoryMaterials)
            if (Array.isArray(categoryMaterials) && categoryMaterials[labelIndex]) {
                existingMaterialData = categoryMaterials[labelIndex];
            } else if (categoryMaterials[label]) {
                // Try to find by label if it's an object
                existingMaterialData = categoryMaterials[label];
            }
        }

        if (existingData?.materials?.main?.[setIndex + 1]?.pillow) {
            const existingPillows = existingData.materials.main[setIndex + 1].pillow;
            console.log(`categoryMaterials:${setIndex}`, categoryMaterials)
            if (Array.isArray(existingPillows) && existingPillows[labelIndex] && existingPillows[labelIndex][subcat.id]) {
                existingMaterialData = existingPillows[labelIndex][subcat.id];
            } else if (existingPillows[label][subcat.id]) {
                // Try to find by label if it's an object
                existingMaterialData = existingPillows[label][subcat.id];
            }
        ...
        if (existingMaterialData && existingMaterialData[1]) {
            ...
            existingMaterialId = activeMaterial.material_id || '';
        }
    ...
    function createMaterialLabelContent(product, variant, category, label, categoryData, roomId, productInstanceId, variantId, existingData, setIndex, labelIndex) {
        const materialGroups = categoryData.materialGroups || {};
        const activeMaterialsList = categoryData.active || [];
        ...
        console.log('Existing material data found:', existingMaterialData);

        if (materialGroups[label]) {
            materialsForLabel = materialGroups[label];
        } else if (activeMaterialsList.length > 0) {
            // If no materialGroups but we have active materials, use the one at the label index
            const numericLabelIndex = parseInt(label.replace(/\D/g, '')) - 1 || 0;
            if (activeMaterialsList[numericLabelIndex]) {
                materialsForLabel = [activeMaterialsList[numericLabelIndex]];
        ...
        // Get existing values - FIXED: Check existingMaterialData structure
        let existingMaterialId = '';
        let existingAreaWeight = '';
        let existingReplacement = '';
        let existingReplacementType = '';

        if (existingMaterialData && Array.isArray(existingMaterialData)) {
            // Structure: [label, material_id, replacement, replacement_type]
            // NO area/weight in the array!
            existingMaterialId = existingMaterialData[1] || '';
            existingReplacement = existingMaterialData[2] || '';
            existingReplacementType = existingMaterialData[3] || '';

            // Area/weight comes from elsewhere (like activeMaterial)
            existingAreaWeight = ''; // Empty by default

            // Try to get from activeMaterial if available
            if (activeMaterial && activeMaterial[measurementField]) {
                existingAreaWeight = activeMaterial[measurementField];
            }
        } else if (existingMaterialData && typeof existingMaterialData === 'object') {
            existingMaterialId = existingMaterialData.material_id || '';
            existingReplacement = existingMaterialData.replacement || '';
            existingReplacementType = existingMaterialData.replacement_type || '';
            existingAreaWeight = existingMaterialData[measurementField] || '';
        } else {
            existingMaterialId = activeMaterial.material_id || '';
        }
        const existingMaterialImage = existingMaterialId ?
            (allMaterials.find(mat => mat.material_id == existingMaterialId)?.material_img || '') :
            '';
        console.log('Parsed existing values:', {
            existingMaterialId,
            existingAreaWeight,
            existingReplacement,
            existingReplacementType
        });
        ...
        if (existingReplacementType) {
            try {
                ...
                if (typeof getMaterials === 'function') {
                    ...
                    console.warn('getMaterials function not found, using empty replacement materials');
                }
            } catch (error) {
        ...
        // Use safe getMaterials if available
        const safeGetMaterials = typeof getMaterials === 'function' ? getMaterials(category) || [] : [];
        ...
        // Build the material select options
        let materialOptions = '<option value="">Select Material</option>';
        if (allMaterials && allMaterials.length > 0) {
            materialOptions += allMaterials.map(material => `
            <option value="${material.material_id}"
                ${(existingMaterialId && parseInt(existingMaterialId) === parseInt(material.material_id)) ? 'selected' : ''}
                data-price="${material.material_price || 0}" 
                data-image="${material.material_img || ''}">
                ${material.material_name}
            </option>
                ...
                <div class="material-compact-fields">
                    ...
                    <div class="material-input${!isCurtainFabric ? ' d-none' : ''}">
                        ...
                        <input type="number" class="form-control curtain-fabric-length"
                            ...
                            data-category="${category}" data-label="${label}" data-variant="${variantId}"
                            value="${existingData?.curtain_data?.[setIndex + 1]?.curtain?.length || ''}"
                            step="0.01" min="0">
                    </div>
                    <div class="material-input${!isCurtainFabric ? ' d-none' : ''}">
                        <label>Height</label>
                        <input type="number" class="form-control curtain-fabric-height"
                            ...
                            data-category="${category}" data-label="${label}" data-variant="${variantId}"
                            value="${existingData?.curtain_data?.[setIndex + 1]?.curtain?.height || ''}"
                            step="0.01" min="0">
                    </div>
                </div>
            </div>
...
<script>
    ...
    function mapDetailImagesToMaterials(detailImages, existingMaterials) {
        ...
        console.log('existingMaterials in mapapi:', detailImages);
