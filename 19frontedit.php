                ...
                <div class="card m-b-30">
                    <div class="card-body">
                        <form id="edit_order_form" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <!-- Same form fields as orderadd.php but pre-populated -->
                                <div class="order-page-col col-md-3 col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="order_date" class="form-control make-datepicker required" id="lblOrderDate" placeholder="<?php echo get_lang_text('orderadd_input_order_date'); ?> *" value="<?php echo date('d.m.Y', strtotime($order->order_date)); ?>">
                                    </div>
                                </div>
                                <div class="order-page-col col-md-3 col-sm-6">
                                    <div class="form-group">
                                        <label for="lblOrderDeliveryDate"><?php echo get_lang_text('orderadd_input_delivery_date'); ?> *</label>
                                        <input type="text" name="order_delivery_date" class="form-control make-datepicker order_delivery_date required" id="lblOrderDeliveryDate" placeholder="<?php echo get_lang_text('orderadd_input_delivery_date'); ?> *" value="<?php echo date('d.m.Y', strtotime($order->order_delivery_date)); ?>">
                                ...
                                <div class="order-page-col col-md-3 col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="order_arcs" class="form-control required" placeholder="<?php echo get_lang_text('orderadd_input_arcs'); ?> *" value="<?php echo htmlspecialchars_decode($order->order_arcs); ?>">
                                    </div>
                                </div>
                                <div class="order-page-col col-md-3 col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <select name="customer_id" id="lblOrderCustomer" class="form-control make-it-select order-select-customer required">
                                            ...
                                            ?>
                                                    <option value="<?php echo $c->customer_id; ?>" data-comm-rate="<?php echo formatExcelPrice($c->customer_comm_rate, 0); ?>" <?php echo $order->customer_id == $c->customer_id ? 'selected' : ''; ?>>
...
<script>
    ...
    function loadOrderData(orderId) {
        $.post(ajax_url + '/api', {
            ...
            alert('Network error loading order data');
        });
    }
    ...
    function populateOrderForm() {
        ...
        const orderInfo = currentOrderData.order_info;

        // Populate basic form fields
        $('#lblOrderDate').val(orderInfo.order_date);
        $('#lblOrderDeliveryDate').val(orderInfo.order_delivery_date);
        $('#lblOrderArcs').val(orderInfo.order_arcs);
        $('#lblOrderCustomer').val(orderInfo.customer_id).trigger('change');
        $('#lblOrderTax').val(orderInfo.order_tax);
        $('#lblOrderExportRegistered').val(orderInfo.order_export_registered);
        $('#lblOrderAgreement').val(orderInfo.agreement_id);
        $('#lblOrderAgreementText').val(orderInfo.agreement_text);
        $('#lblOrderNotes').val(orderInfo.order_notes);

        // Load customer address
        setTimeout(() => {
            $('#lblOrderCustomerAddress').val(orderInfo.address_id).trigger('change');
        }, 500);

        // Populate rooms and products
        populateRoomsAndProducts();
    }

    // Function to populate rooms and products
    function populateRoomsAndProducts() {
        if (!currentOrderData.order_details || currentOrderData.order_details.length === 0) return;
        ...
        Object.values(rooms).forEach((room, index) => {
            ...
            createRoomFromData(room, roomNumber);
        });

        // Update totals with longer delay to ensure everything is loaded
        setTimeout(updateOrderTotals, 3000);
    }

    // Function to create room from data
    function createRoomFromData(roomData, roomNumber) {
        if (roomNumber === 1) {
        ...
        roomData.products.forEach(productData => {
            setTimeout(() => {
                addProductToRoomForEdit(roomData.room_id, productData);
            }, 100);
        });
    }
    ...
    // Function to add product to room in edit mode
    function addProductToRoomForEdit(roomId, productData) {
    ...
    function addProductTab(roomId, product, existingData = null) {
        ...
        setTimeout(() => {
            loadProductContent(productId, product, roomId, existingData);
    ...
    function loadProductContent(contentId, product, roomId, existingData = null) {
        ...
        if (product.has_variant && product.available_in === 'set') {
            $.post(ajax_url + '/api', {
                ...
                if (response.status === 'success') {
                    ...
                    if (existingData) {
                        setTimeout(() => {
                            populateProductData(product.product_id, roomId, existingData);
                        }, 500);
                    }
                }
            });
    ...
    // Function to populate product data with existing values
    function populateProductData(productId, roomId, existingData) {
        const $productContent = $(`#product-${productId}-room${roomId}`);
        ...
        // Populate items for fitout products
        ...
        // Trigger calculation after populating data
        setTimeout(() => {
...
<script>
    ...
    $(document).on('input change', '.dimension-width, .dimension-length, .dimension-height, .product-qty, .product-discount', function() {
        const $productContent = $(this).closest('.product-content, .variant-details');
        if ($productContent.length) {
            const productId = $productContent.closest('[id*="product-"]').attr('id').replace('product-', '').replace(/-room\d+$/, '');
            const roomId = $productContent.closest('[id*="room"]').attr('id').match(/room(\d+)/)[1];
    ...
    // Enhanced surcharge handler
    $(document).on('input change', '.surcharge-checkbox', function() {
        const $productContent = $(this).closest('.product-content, .variant-details');
        if ($productContent.length) {
            const productId = $productContent.closest('[id*="product-"]').attr('id').replace('product-', '').replace(/-room\d+$/, '');
            const roomId = $productContent.closest('[id*="room"]').attr('id').match(/room(\d+)/)[1];
