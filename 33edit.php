...
<script>
    ...
    function calculateMaterialCosts($contentElement) {
        ...
        $contentElement.find('.material-group .material-inputs-compact').each(function() {
            ...
            const standardMaterialUnitPrice = parseFloat($materialGroup.find('.material-type-select option:selected').data('standard-material-price')) || 0;
            console.log('materials:', {
                areaWeight: areaWeight,
                fabricHeight: fabricHeight,
                fabricLength: fabricLength,
                unitPrice: unitPrice,
                standardMaterialUnitPrice: standardMaterialUnitPrice
            });
            ...
            console.log('materials cost non pillow:', {
                materialTotal: materialTotal
            });
        });
        console.log('final materials cost non pillow:', {
            materialTotal: materialTotal
        });
        ...
        $contentElement.find('.pillow-material-group .pillow-material-inputs-compact').each(function() {
            ...
            const selectedMatPrice = parseFloat($pillowGroup.find('.material-type-select option:selected').data('standard-material-price')) || 0;
            let selectedMatPrdPrice = 0;
            let newMatPrdPrice = 0;

            let mt_of = $pillowGroup.data('mt_of') || '';
            // console.log('mt_of=>'+mt_of);
            let pipingLength = 0;
            let area = 0;
            ...
            if (mt_of === "default") {
                // Double-sided pillow price calculation
                selectedMatPrdPrice = selectedMatPrice * area * quantity * 2;
                newMatPrdPrice = newMaterialPrice * area * quantity * 2;

                //  console.log("   - Selected Pillow Price (Old):", selectedMatPrdPrice);
                //  console.log("   - New Pillow Price (New):", newMatPrdPrice);
            } else {
                if (mt_of === "piping") {
                ...
                console.log("   - selected Material Price:", selectedMatPrice);
                console.log(
                    "   - Total Selected Material Price (Old):",
                    selectedMatPrdPrice
                );
                console.log("   - Total New Material Price (New):", newMatPrdPrice);
            }
            materialTotal += (newMatPrdPrice - selectedMatPrdPrice);
        });
        console.log('final materials cost with pillow:', {
            materialTotal: materialTotal
        });
...
<script>
                    ...
                    const productPromise = new Promise((productResolve, productReject) => {
                        ...
                        setTimeout(() => {
                            addProductToRoomForEditWithPromise(roomData.room_id, productData)
                                .then(() => {
                                    setTimeout(() => {
                                        if ($(".make-it-select").length > 0) {
                                            $(".make-it-select").select2();
                                        }
                                    }, 100);
    ...
    $(document).on('change', '.material-type-replacement', function() {
        ...
        if (selectedMaterialType) {
            ...
            // Pass category to loadMaterials function
            loadMaterials($materialSelect, selectedMaterialType);
        } else {
            // Reset to default if no material type selected
            $materialSelect.html('<option value="">Select Material</option>');
            // Optionally reload with original availableMaterials
    ...
    function loadMaterials(selectElement, category) {
        const selectedMaterial = selectElement.find('option:selected').val();
        const standard_material_price = selectElement.find('option:selected').data('standard-material-price');
        $.post(ajax_url + '/api', {
            get_materials_by_category: 1,
            category: category,
            selected_material: selectedMaterial,
            standard_material_price: standard_material_price
        }, function(data) {
            // console.log(data);
            var response = $.parseJSON(data);
            if (response.status === 'success') {
...
<script>
    ...
    // Function to create material inputs for a specific material group
    async function createMaterialGroupInputs(product, variant, category, refLabel, materials, roomId, variantId, categoryData, existingMaterials, existingCurtainData, index = 0) {
        ...
        const standardMaterialPrice = availableMaterials.find(mat => mat.material_id == activeMaterial?.material_id)?.material_price || 0;
            ...
            for (const material of categoryMaterials) {
                if (material[0] === refLabel) {
                    existingMaterial = {
                        ...
                        mt_type: material[3] || '',
                        material_img: availableMaterials.find(mat => mat.material_id == material[1])?.material_img || '',
        ...
        // Get initial replacement materials if replacement type exists
        let replacementMaterials = [];
        if (currentMaterial?.mt_type) {
            replacementMaterials = await getMaterials(currentMaterial.mt_type);
        }
            ...
            <div class="material-grid">
                <div class="material-inputs-compact">
                    ...
                    <div class="material-input">
                            ...
                            <select class="form-control make-it-select material-type-select" 
                ...
                <div class="material-inputs-compact-replacement">
                    ...
                    <div class="material-compact-fields">
                        <div class="material-input">
                            ...
                            ${['fabric','glass','metal','wood'].filter(material_type => material_type !== category).map(material_type => `
                                <option value="${material_type}" ${(currentMaterial?.mt_type||'') === material_type?'selected':''}>
                                        ${material_type.charAt(0).toUpperCase() + material_type.slice(1)}
                                </option>
                            `).join('')}
                            </select>
                            <select class="form-control make-it-select material-replacement" 
                                data-variant="${variantId}" data-category="${category}" data-ref-label="${refLabel}">
                                <option value="">Select Material</option>
                                ${replacementMaterials.map(material => `
                                    <option value="${material.material_id}" 
                                        ${currentMaterial?.replacement == material.material_id ? 'selected' : ''}
                                        data-image="${material.material_img || ''}"
    ...
    // Function to get materials by category (async)
    async function getMaterials(category) {
        try {
            const response = await $.post(ajax_url + '/api', {
                get_materials_by_category: 1,
                category: category
            });
    ...
    // Event handler for replacement type change
    $(document).on('change', '.material-type-replacement', async function() {
        const materialType = $(this).val();
        const $replacementSelect = $(this).closest('.material-input').find('.material-replacement');
        const variantId = $(this).data('variant');
        const category = $(this).data('category');
        const refLabel = $(this).data('ref-label');

        if (!materialType) {
            $replacementSelect.html('<option value="">Select Material</option>');
            return;
        }

        // Show loading
        $replacementSelect.html('<option value="">Loading materials...</option>');

        try {
            // Fetch materials for the selected type
            const materials = await getMaterials(materialType);

            // Update the replacement select
            let options = '<option value="">Select Material</option>';
            materials.forEach(material => {
                options += `<option value="${material.material_id}" 
                data-image="${material.material_img || ''}"
                data-name="${material.material_name || ''}">
                ${material.material_name}
            </option>`;
            ...
            $replacementSelect.html(options);
        } catch (error) {
            console.error('Error loading replacement materials:', error);
            $replacementSelect.html('<option value="">Error loading materials</option>');
        }
    });

    // Event handler for replacement material change to update image
               ...
               <div class="material-inputs-compact-replacement">
                    ...
                    <div class="material-input">
                        ...
                        <select class="form-control make-it-select material-replacement" 
                                data-variant="${variantId}" data-category="${category}">
                                <option value="">Select Material</option>
                                <option value="${currentMaterial.replacement || ''}" 
                                    data-image="${currentMaterial.replacement_image || ''}" selected>
                                    ${currentMaterial.replacement_material_name}
                                </option>
                        </select>
                    </div>
                    </div>
    ...
    function createPillowMaterialGroup(product, variant, subcat, refLabel, materials, allMaterials, roomId, variantId, existingMaterials, index = 0) {
            ...
            for (const material of pillowMaterials) {
                if (material[0] === refLabel) {
                    ...
                    existingMaterial = {
                        ...
                        material_img: allMaterials.find(mat => mat.material_id == material[1][subcat.id])?.material_img || '',
        ...
        return `
            <div class="pillow-material-group" data-ref-label="${refLabel}" data-mt_of="${subcat.id}">
    ...
    function createVariantsRadioSelection(product, variants, roomId, existingData, productInstanceId) {
        ...
        const bed_dim = existingData && existingData.bed_dim ? existingData.bed_dim : (existingData.attr_bed_dim ? '180200' : '0');
    ...
    function createMaterialSection(product, variant, roomId, existingData, productInstanceId, levelIndex = 0) {
                  ...
                  <div class="material-tabs-content" id="materialTabsContent-${productInstanceId}-${variantId}-room${roomId}">
                     ${materialCategories.map((category, index) => `
                        <div class="material-tab-content ${index === 0 ? 'active' : ''}" 
                           ...
                           ${category.id === 'pillow' ? 
                                 createPillowSubcategoriesForVariant(product, variant, roomId, activeMaterials, existingMaterials, productInstanceId, levelIndex) : 
                                 createStandardMaterialContent(product, variant, category.id, roomId, activeMaterials, existingMaterials, existingCurtainData, levelIndex)
                           }
                        </div>
                     `).join('')}
                  </div>
    ...
    function collectMaterialsDataNewLayout(productId, roomId, variantId = null, itemId = null) {
        ...
        $container.find('.material-tab-content').each(function() {
            ...
            $materialTab.find('.pillow-material-group').each(function() {
                ...
                const standardMaterialUnitPriceFace = parseFloat($pillowGroup.find('.material-type-select[data-subcategory="front"] option:selected').data('standard-material-price')) || 0;
                const standardMaterialUnitPriceBack = parseFloat($pillowGroup.find('.material-type-select[data-subcategory="back"] option:selected').data('standard-material-price')) || 0;
                const standardMaterialUnitPricePiping = parseFloat($pillowGroup.find('.material-type-select[data-subcategory="pipping"] option:selected').data('standard-material-price')) || 0;
                const standardMaterialUnitPriceDefault = parseFloat($pillowGroup.find('.material-type-select[data-subcategory="default"] option:selected').data('standard-material-price')) || 0;
                ...
                if (Object.keys(pillowData).length > 0) {
                    ...
                    materials.main[setLevel]['pillow'].push([
                        ...
                        replacement,
                        standardMaterialUnitPrice,
                        standardMaterialUnitPriceDefault,
                        standardMaterialUnitPriceFace,
                        standardMaterialUnitPriceBack,
                        standardMaterialUnitPricePiping,
