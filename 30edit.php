...
<style>
    ...
    .modal-fullscreen .multi-select-modal-body {
...
<script>
    ...
    // Optimize material cost calculation
    function calculateMaterialCosts($contentElement) {
        let materialTotal = 0;

        // Combine similar selectors to reduce DOM queries
        $contentElement.find('.material-group, .material-inputs-compact').each(function() {
            ...
            const unitPrice = parseFloat($materialGroup.find('.material-type-select option:selected').data('price')) || 0;
            if (fabricLength > 0 && fabricHeight > 0) {
                const fabricArea = (fabricLength * fabricHeight) / 10000;
        ...
        $contentElement.find('.pillow-material-group, .pillow-material-inputs-compact').each(function() {
            ...
            const unitPrice = parseFloat($pillowGroup.find('.material-type-select option:selected').data('price')) || 0;

            const pillowArea = (length * width) / 10000;
...
<script>
            ...
            try {
                ...
                currentOrderData.order_details.forEach(detail => {
                    ...
                    rooms[roomId].products.push({
                        ...
                        // available_in: detail.product_data?.available_in || 'simple',
                        // unit_price: detail.unit_price,
                        // calculate_type: detail.calculate_type,
                        // product_data: detail.product_data,
                    });
                });
    ...
    function addProductToRoomForEditWithPromise(roomId, productData) {
        state.currentProductId = productData.product_id;
        return new Promise((resolve, reject) => {
    ...
    // Update the loadProductContent function to use Promise
    function loadProductContentWithPromise(contentId, product, roomId, existingData = null) {
            ...
            if (product.has_variant && product.available_in === 'set') {
                $.post(ajax_url + '/api', {
                    ...
                    if (response.status === 'success') {
                        ...
                        console.log('before loadProductWithVariants:', {
                            productId: product.product_id,
                            variants: variants,
                            roomId: roomId,
                            existingData: existingData
                        });
                        loadProductWithVariants($content, product, variants, roomId, existingData);
                        console.log('after loadProductWithVariants:', {
                            productId: product.product_id,
                            variants: variants
                        });
                                    ...
                                    existingData.curtain_data[index + 1].accessories.forEach(accessory => {
                                        ...
                                        $.post(ajax_url + '/api', {
                                            ...
                                            if (response.status === 'success') {
                                                ...
                                                if (accessoryOptions) {
                                                    addAccessoryToCurtainVariant(roomId, product.product_id, variant.product_id, curtainAccessoryTypes.find(type => type.id === accessory.type), accessoryOptions, accessory, blackout_color);
                                                } else {
                        ...
                        if (existingData) {
                            setTimeout(() => {
                                calculateProductTotal(product.product_id, roomId);
                                resolve();
                            }, 1000);
                        }
                    } else {
                ...
                if (product.type === 'fitout') {
                    loadFitoutProductContent($content, product, roomId, existingData);
                        ...
                        existingData.items.forEach(existingItem => {
                            $.post(ajax_url + '/api', {
                                ...
                                if (response.status === 'success') {
                                    ...
                                    if (selectedItem) {
                                        addItemToProduct(roomId, product.product_id, selectedItem, existingItem);
                                        setTimeout(() => {
                                            calculateProductTotal(product.product_id, roomId, 'fitout');
                                        }, 1000);
                                    } else {
                    ...
                    loadCurtainProductContent($content, product, product, roomId, existingData);
                } else {
                    loadSimpleProductContent($content, product, product, roomId, existingData);
                }
                    ...
                    existingData.curtain_data[1].accessories.forEach(accessory => {
                        ...
                        $.post(ajax_url + '/api', {
                            ...
                            if (response.status === 'success') {
                                ...
                                if (accessoryOptions) {
                                    addAccessoryToProduct(roomId, product.product_id, curtainAccessoryTypes.find(type => type.id === accessory.type), accessoryOptions, accessory, blackout_color);
                                } else {
                                    console.error('Could not find accessories');
                                }
                            }
    ...
    function addProductTab(roomId, product, existingData = null) {
        ...
        // Generate unique instance ID by counting existing instances
        const existingInstances = $tabsContainer.find(`[data-product="${product.product_id}"]`).length;
        const instanceNumber = existingInstances + 1;
        const instanceId = `${product.product_id}-instance${instanceNumber}`;
        ...
        const $tab = $(`
        <div class="product-tab" data-product="${product.product_id}" data-type="${product.type}" data-available-in="${product.available_in}" data-detail-id="${product.detail_id || ''}" id="${tabId}">
    ...
    `);
        ...
        return new Promise((resolve) => {
            ...
            setTimeout(() => {
                loadProductContentWithPromise(productId, product, roomId, existingData)
                    .then(() => {
                        // Calculate total after content is loaded
                        setTimeout(() => {
                            console.log('Calculating total for product:', product.product_id, 'room:', roomId);
                            calculateProductTotal(product.product_id, roomId);
                            resolve();
                        }, 1500);
                    });
    ...
    // Enhanced function to populate materials data
    function populateMaterialsData(productId, roomId, materials) {
        console.log('Populating materials data:', {
            productId,
            roomId,
            materials
        });

        // Handle main materials
        if (materials.main) {
            Object.keys(materials.main).forEach(setLevel => {
                const categories = materials.main[setLevel];
                console.log(`Processing set level ${setLevel}:`, categories);

                Object.keys(categories).forEach(category => {
                    const materialGroups = categories[category];
                    console.log(`Processing category ${category}:`, materialGroups);

                    materialGroups.forEach(materialGroup => {
                        if (Array.isArray(materialGroup) && materialGroup.length >= 2) {
                            const [refLabel, materialId, replacement] = materialGroup;

                            console.log(`Processing material group - Ref: ${refLabel}, Material: ${materialId}`);

                            // Build selector for this material group
                            const selector = `.material-type-select[data-category="${category}"][data-ref-label="${refLabel}"]`;
                            const $materialSelect = $(`#product-${productId}-room${roomId} ${selector}`);

                            if ($materialSelect.length) {
                                console.log(`Found material select for ${category} - ${refLabel}, setting value: ${materialId}`);
                                $materialSelect.val(materialId).trigger('change');

                                // Populate area/weight if available (you might need to get this from your data)
                                const areaWeightSelector = `.area-weight[data-category="${category}"][data-ref-label="${refLabel}"]`;
                                const $areaWeight = $(`#product-${productId}-room${roomId} ${areaWeightSelector}`);

                                // If you have area/weight data in your materials, populate it here
                                // $areaWeight.val(areaWeight);
                            } else {
                                console.warn(`Material select not found for selector: ${selector}`);

                                // Try alternative selectors
                                const altSelectors = [
                                    `.material-type-select[data-ref-label="${refLabel}"]`,
                                    `.material-type-select[data-category="${category}"]`,
                                    `.material-type-select`
                                ];

                                for (const altSelector of altSelectors) {
                                    const $altSelect = $(`#product-${productId}-room${roomId} ${altSelector}`);
                                    if ($altSelect.length) {
                                        console.log(`Found alternative selector: ${altSelector}, setting value: ${materialId}`);
                                        $altSelect.val(materialId).trigger('change');
                                        break;
                                    }
                                }
                            }
                        } else if (typeof materialGroup === 'object') {
                            // Handle pillow materials with multiple parts
                            console.log('Processing pillow material:', materialGroup);
                            // You'll need specific logic for pillow materials
                        }
                    });
                });
            });
    ...
    // Function to populate surcharges data
    function populateSurchargesData(productId, roomId, surcharges) {
        console.log('Populating surcharges data:', {
            productId,
            roomId,
            surcharges
        });
        ...
        surcharges.forEach(surcharge => {
            if (surcharge.applied) {
                // Try different selector patterns to find the checkbox
                const selectors = [
                    `#product-${productId}-room${roomId} .surcharge-checkbox[data-surcharge-name="${surcharge.name}"]`,
                    `#product-${productId}-room${roomId} .surcharge-checkbox[data-surcharge-rate="${surcharge.rate}"]`,
                    `.surcharge-checkbox[data-surcharge-name="${surcharge.name}"]`
                ];

                let $checkbox = null;
                for (const selector of selectors) {
                    $checkbox = $(selector);
                    if ($checkbox.length) break;
                }

                if ($checkbox.length) {
                    $checkbox.prop('checked', true);
                    console.log('Applied surcharge:', surcharge.name);
                } else {
                    console.warn('Surcharge checkbox not found:', surcharge.name);
                }
            }
        });
    }
    ...
    function setupFitoutCalculationHandlers() {
        // Fitout base product calculations
        // $(document).on('input change', '.product-content[data-type="fitout"] .dimension-width, .product-content[data-type="fitout"] .dimension-length, .product-content[data-type="fitout"] .product-qty, .product-content[data-type="fitout"] .product-discount', function() {
        ...
        //         console.log('Fitout base product change detected, recalculating...');
    ...
    $(document).on('click', '#loadMoreItemsBtn', function() {
        applyItemFilters(qualifications);
    });
...
<script>
    ...
    function createVariantsTabs(product, variants, roomId, existingData) {
        return `
               ...
               <div class="product-variants-content" id="variants-content-${product.product_id}-room${roomId}">
                  ${variants.map((variant, index) => `
                     <div class="product-variant-content ${index === 0 ? 'active' : ''}" 
                        ...
                        ${createVariantContentForSet(product, variant, roomId, existingData, index)}
                     </div>
                  `).join('')}
               </div>
            </div>
    ...
    function createMaterialInputsForCategory(product, variant, category, roomId, activeMaterials, existingMaterials, existingCurtainData = null, index = 0) {
        ...
        if (categoryData.materialGroups && Object.keys(categoryData.materialGroups).length > 0) {
            return Object.entries(categoryData.materialGroups).map(([refLabel, materials]) => {
                ...
                return createMaterialGroupInputs(product, variant, category, refLabel, materials, roomId, variantId, categoryData, existingMaterials, existingCurtainData, index);
            }).join('');
        } else {
            // Fallback to single material input using active materials
            return createSingleMaterialInputs(product, variant, category, roomId, variantId, categoryData, existingMaterials, existingCurtainData, index);
        }
    }

    // Function to create material inputs for a specific material group (A, B, C, etc.)
    function createMaterialGroupInputs(product, variant, category, refLabel, materials, roomId, variantId, categoryData, existingMaterials, existingCurtainData, index = 0) {
        ...
        if (existingMaterials?.main && existingMaterials.main[index + 1] && existingMaterials.main[index + 1][category]) {
            const categoryMaterials = existingMaterials.main[index + 1][category];
            // Find material with matching refLabel
            for (const material of categoryMaterials) {
                if (material[0] === refLabel) {
                    existingMaterial = {
                        refLabel: material[0],
                        material_id: material[1],
                        replacement: material[2] || '',
                        material_image: availableMaterials.find(mat => mat.material_id == material[1])?.material_img || '',
                        material_name: availableMaterials.find(mat => mat.material_id == material[1])?.material_name || '',
                        alias_name: availableMaterials.find(mat => mat.material_id == material[1])?.alias_name || '',
                        [measurementField]: material[3] || '',
                    };
                    break;
                }
            }
        ...
        const currentMaterial = existingMaterial ? existingMaterial : activeMaterial;
        const fabricLength = existingCurtainData && existingCurtainData[index + 1] ? existingCurtainData[index + 1].curtain.length : '';
        const fabricHeight = existingCurtainData && existingCurtainData[index + 1] ? existingCurtainData[index + 1].curtain.height : '';

        return `
            <div class="material-group" data-ref-label="${refLabel}">
                  <div class="material-group-header">
                     <h6>Material ${refLabel} ${currentMaterial?.alias_name ? `- ${currentMaterial.alias_name}` : ''}</h6>
                  </div>
                  <div class="material-inputs-compact">
                     <div class="material-compact-image">
                        ${currentMaterial?.material_img ? 
                              `<img src="<?= URL ?>/uploads/material/${currentMaterial.material_img}" alt="${currentMaterial.material_name}" style="width:100%;height:100%;object-fit:cover;">` : 
                              `<i class="fa fa-image"></i>`
                        }
                     </div>
                     <div class="material-compact-fields">
                              ...
                              <select class="form-control material-type-select" 
                                 ...
                                 ${availableMaterials.map(material => `
                                    <option value="${material.material_id}" 
                                          ${currentMaterial && currentMaterial.material_id == material.material_id ? 'selected' : ''}
                        ...
                        <div class="material-input d-none">
                              ...
                              <input type="number" class="form-control area-weight" 
                                    ...
                                    data-variant="${variantId}" data-category="${category}" data-ref-label="${refLabel}"
                                    value="${currentMaterial ? currentMaterial[measurementField] || '' : ''}"
                                    step="0.01" min="0">
                        </div>
    ...
    function createSingleMaterialInputs(product, variant, category, roomId, variantId, categoryData, existingMaterials, existingCurtainData, index = 0) {
    ...
    function createStandardMaterialContent(product, variant, category, roomId, activeMaterials, existingMaterials, existingCurtainData, index) {
        return createMaterialInputsForCategory(product, variant, category, roomId, activeMaterials, existingMaterials, existingCurtainData, index);
    }

    // Updated function to create pillow subcategories with material groups
    function createPillowSubcategoriesForVariant(product, variant, roomId, activeMaterials, existingMaterials, index = 0) {
        const variantId = product.available_in === 'size' ? variant.id : variant.product_id;
                ...
                <div class="pillow-subcategories-content" id="pillowContent-${product.product_id}-${variantId}-room${roomId}">
                    ${subcategories.map((subcat, index) => `
                        ...
                        <div class="pillow-subcategory-details">
                            ${createPillowMaterialContent(product, variant, subcat, pillowData, roomId, variantId, existingMaterials, index)}
                        </div>
                        </div>
                    `).join('')}
                </div>
    ...
    function createPillowMaterialContent(product, variant, subcat, pillowData, roomId, variantId, existingMaterials, index = 0) {
        ...
        if (Object.keys(materialGroups).length > 0) {
            return Object.entries(materialGroups).map(([refLabel, materials]) => {
                return createPillowMaterialGroup(product, variant, subcat, refLabel, materials, allMaterials, roomId, variantId, existingMaterials, index);
            }).join('');
        }
        // If we have active materials but no groups, create single input
        else if (activeMaterials.length > 0) {
            return createSinglePillowMaterialInput(product, variant, subcat, activeMaterials[0], allMaterials, roomId, variantId, existingMaterials, index);
        }
        // If no active materials but have available materials, create empty input
        else if (allMaterials.length > 0) {
            return createSinglePillowMaterialInput(product, variant, subcat, null, allMaterials, roomId, variantId, existingMaterials, index);
        }
    ...
    function createPillowMaterialGroup(product, variant, subcat, refLabel, materials, allMaterials, roomId, variantId, existingMaterials, index = 0) {
        const activeMaterial = materials && materials.length > 0 ? materials[0] : null;
        let existingMaterial = null;
        if (existingMaterials && existingMaterials.main && existingMaterials.main[index + 1] && existingMaterials.main[index + 1]['pillow']) {
            const pillowMaterials = existingMaterials.main[index + 1]['pillow'];
            for (const material of pillowMaterials) {
                if (material[0] === refLabel) {
                    existingMaterial = {
                        refLabel: material[0],
                        material_id: material[1][subcat.id] || '',
                        alias_name: allMaterials.find(mat => mat.material_id == material[1][subcat.id])?.alias_name || '',
                        material_image: allMaterials.find(mat => mat.material_id == material[1][subcat.id])?.material_img || '',
                        material_name: allMaterials.find(mat => mat.material_id == material[1][subcat.id])?.material_name || '',
                        quantity: material[2] || 1,
                        length_cm: material[3] || '',
                        width_cm: material[4] || ''
                    };
                    break;
                }
        ...
        return `
            <div class="pillow-material-group" data-ref-label="${refLabel}">
                  <div class="pillow-material-group-header">
                     <h6>${subcat.name} - ${refLabel} ${currentMaterial?.alias_name ? `- ${currentMaterial.alias_name}` : ''}</h6>
                  </div>
                        ...
                        ${subcat.id === 'default_pillow' ? `
                              <div class="pillow-material-input d-none">
                                 ...
                                 <input type="number" class="form-control pillow-quantity" 
                                       placeholder="1" min="1" value="${currentMaterial?.quantity || 1}"
                                       data-subcategory="${subcat.id}" data-variant="${variantId}" data-ref-label="${refLabel}">
                              </div>
                              <div class="pillow-material-input d-none">
                                 ...
                                 <div class="pillow-dimensions d-flex align-items-center">
                                    <input type="number" class="form-control pillow-length" 
                                          ...
                                          value="${currentMaterial?.length_cm || currentMaterial?.dimensions?.length || ''}"
                                          data-subcategory="${subcat.id}" data-variant="${variantId}" data-ref-label="${refLabel}">
                                    <span class="dimension-separator">×</span>
                                    <input type="number" class="form-control pillow-width" 
                                          placeholder="Width" step="0.1" min="0"
                                          value="${currentMaterial?.width_cm || currentMaterial?.dimensions?.width || ''}"
                                          data-subcategory="${subcat.id}" data-variant="${variantId}" data-ref-label="${refLabel}">
                                 </div>
                              </div>
                        ` : ''}
    ...
    function createCurtainVariantContent(product, variant, roomId, existingData, index) {
        ...
        console.log('in loadCurtainProductContent, open_with:', open_with);
        console.log('in loadCurtainProductContent, opening_direction:', opening_direction);
        console.log('in loadCurtainProductContent, installation_needed:', installation_needed);
        ...
        return `
            <div class="variant-details">
                <div class="compact-product-details">
                        ...
                        <div class="compact-details-fields">
                            ...
                            <div class="compact-detail-group" style="width:200px;">
                                ...
                                <textarea class="form-control product-notes-tr" 
                                        ...
                                        data-variant="${variant.product_id}">${existingData?.product_notes_tr || ''}</textarea>
                            </div>
                            <div class="compact-detail-group" style="width:200px;">
                                <label>Notes(EN)</label>
                                <textarea class="form-control product-notes-en" 
                                        placeholder="Enter notes in English"
                                        data-variant="${variant.product_id}">${existingData?.product_notes_en || ''}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    ${createMaterialSection(product, variant, roomId, existingData, index)}
                        ...
                        <div class="curtain-control installation-control">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input curtain-installation-needed-checkbox" 
                                        id="curtain-installation${product.product_id}${variant.product_id}${roomId}" 
                                        value="needed" ${installation_needed ? 'checked' : ''}
                                        data-variant="${variant.product_id}">
                                <label class="custom-control-label font-weight-bold" 
                                        for="curtain-installation${product.product_id}${variant.product_id}${roomId}">
    ...
    function setupCurtainOptions(product, variants, roomId) {
    ...
    function createVariantContentForSet(product, variant, roomId, existingData, index) {
        // Check if this is a curtain product
        const isCurtainProduct = product.type === 'curtain';

        if (isCurtainProduct) {
            return createCurtainVariantContent(product, variant, roomId, existingData, index);
        } else {
            const dims = variant.dims || {}; // fallback if undefined
    ...
    function createVariantContentForSize(product, variant, roomId, existingData, index) {
        ...
        if (existingData && existingData.attr_bed_dim && variant.id === existingData.bed_dim) {
            width = existingData.attr_dims.width;
            length = existingData.attr_dims.length;
            height = existingData.attr_dims.height;
        }
        const quantity = existingData?.quantity || 1;
        const discount = existingData?.discount || 0;
        return `
                    ...
                    <div class="compact-details-with-image">
                        ...
                        <div class="compact-details-fields">
                                ...
                                <div class="compact-detail-group" style="width:200px;">
                                    ...
                                    <textarea class="form-control product-notes-tr" 
                                        placeholder="Enter notes in Turkish" 
                                        data-variant="${variant.id}">${existingData?.product_notes_tr || ''}</textarea>
                                </div>
                                <div class="compact-detail-group" style="width:200px;">
                                    <label>Notes(EN)</label>
                                    <textarea class="form-control product-notes-en" 
                                    placeholder="Enter notes in English" 
                                    data-variant="${variant.id}">${existingData?.product_notes_en || ''}</textarea>
                                </div>
                        </div>
                    </div>
    ...
    function setupVariantsTabs(product, variants, roomId) {
        ...
        if (product.type === 'curtain') {
            setupCurtainOptions(product, variants, roomId);
        }
    ...
    // function to setup material tabs for product
    ...
    function loadProductWithVariants($content, product, variants, roomId, existingData) {
        ...
        if (product.available_in == 'set') {
            ...
            variantsHTML = createVariantsTabs(product, variants, roomId, existingData);
        } else {
            variantsHTML = createVariantsRadioSelection(product, variants, roomId, existingData);
        }
    ...
    function createVariantsRadioSelection(product, variants, roomId, existingData) {
        const productId = product.product_id; // Define productId here
        const bed_dim = existingData && existingData.bed_dim ? existingData.bed_dim : '180200';
        ...
        return `
            <div class="product-variants-section" id="variants-section-${productId}-room${roomId}">
               ...
               <div class="product-variants-content" id="variants-content-${productId}-room${roomId}">
                  ${variants.map((variant, index) => `
                     <div class="product-variant-content ${variant.id === bed_dim ? 'active' : ''}" 
                        id="variant-${productId}-${variant.id}-room${roomId}">
                        ${createVariantContentForSize(product, variant, roomId, existingData, index)}
                     </div>
                  `).join('')}
               </div>
            </div>
    ...
    function loadFitoutProductContent($content, product, roomId, existingData) {
        ...
        if (existingData && existingData.attr_dims) {
            const attrDims = existingData.attr_dims;
            if (attrDims.width) {
    ...
    function loadSimpleProductContent($content, product, variant, roomId, existingData = null) {
        ...
        const $wrapper = $(`
            <div class="simple-product-content">
                  <div class="compact-product-details">
                     ...
                     <div class="compact-details-with-image">
                        ...
                        <div class="compact-details-fields">
                              ...
                              <div class="compact-detail-group">
                                 <label>Notes(TR)</label>
                                 <textarea class="form-control product-notes-tr" 
                                    placeholder="0" step="0.01" min="0" value="${notes_tr}"></textarea>
                              </div>
                              <div class="compact-detail-group">
                                 <label>Notes(EN)</label>
                                 <textarea class="form-control product-notes-en" 
                                    placeholder="0" step="0.01" min="0" value="${notes_en}"></textarea>
                              </div>
                        </div>
                     </div>
                     <!-- Add base price field for variant -->
                     ...
                     <!-- Material Section - Use existing data -->
                     ${createMaterialSection(product, variant, roomId, existingData)}
    ...
    function loadCurtainProductContent($content, product, variant, roomId, existingData) {
        ...
        const productKey = `curtain-${product.product_id}-room${roomId}`;
                    ...
                    <div class="curtain-options-section">
                        ...
                        <div class="curtain-controls">
                            ...
                            <div class="curtain-control installation-control">
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" class="custom-control-input curtain-installation-needed-checkbox" 
                                        id="curtain-installation${product.product_id}${roomId}" 
                                        value="needed" ${installation_needed ? 'checked' : ''}>
                                    <label class="custom-control-label font-weight-bold" 
                                        for="curtain-installation${product.product_id}${roomId}">
                                        <i class="fa fa-tools mr-2"></i>Installation Needed
                                        <i class="fa fa-info-circle text-muted ml-1" 
                                        data-toggle="tooltip" 
                                        data-placement="top" 
        ...
        setupCurtainProductHandlers(product, roomId);
    ...
    function setupCurtainProductHandlers(product, roomId) {
        const productId = product.product_id;
        const $productContent = $(`#product-${productId}-room${roomId}`);
        ...
        $curtainSection.on('change', '.curtain-installation-needed-checkbox', function() {
            const $checkbox = $(this);
            updateCurtainInstallationPrice($checkbox);
            // No need to call calculateProductTotal here as updateCurtainInstallationPrice already does it
        });

        // Setup material tabs after content is loaded
        setTimeout(() => {
            setupMaterialTabsForProduct(product, product, roomId);
            setupPillowSubcategoryTabs(product, product, roomId);
        }, 100);
    }
    ...
    function createMaterialSection(product, variant, roomId, existingData, index = 0) {
        ...
        const existingCurtainData = existingData && existingData.curtain_data ? existingData.curtain_data : {};
                  ...
                  <div class="material-tabs-content" id="materialTabsContent-${product.product_id}-${variantId}-room${roomId}">
                     ${materialCategories.map((category, index) => `
                        <div class="material-tab-content ${index === 0 ? 'active' : ''}" 
                           ...
                           ${category.id === 'pillow' ? 
                                 createPillowSubcategoriesForVariant(product, variant, roomId, activeMaterials, existingMaterials, index) : 
                                 createStandardMaterialContent(product, variant, category.id, roomId, activeMaterials, existingMaterials, existingCurtainData, index)
                           }
                        </div>
                     `).join('')}
                  </div>
    ...
    function createMaterialSectionForItem(item, productId, roomId, existingData, index = 0) {
                  ...
                  <div class="material-tabs-content" id="materialTabsContent-${itemId}-${productId}-room${roomId}">
                     ${materialCategories.map((category, index) => `
                        <div class="material-tab-content ${index === 0 ? 'active' : ''}" 
                           ...
                           ${category.id === 'pillow' ? 
                                 createPillowSubcategoriesForItem(item, productId, roomId, activeMaterials, existingMaterials) : 
                                 createStandardMaterialContentForItem(item, category.id, roomId, activeMaterials, existingMaterials)
                           }
                        </div>
                     `).join('')}
                  </div>
    ...
    function createStandardMaterialContentForItem(item, category, roomId, activeMaterials, existingMaterials) {
        return createMaterialInputsForCategory(item, item, category, roomId, activeMaterials, existingMaterials);
    }

    // Function to create pillow subcategories for items
    function createPillowSubcategoriesForItem(item, productId, roomId, activeMaterials, existingMaterials) {
        const itemId = item.product_id;
                  ...
                  <div class="pillow-subcategories-content" id="pillowContent-${itemId}-${productId}-room${roomId}">
                     ${subcategories.map((subcat, index) => `
                        <div class="pillow-subcategory-content ${index === 0 ? 'active' : ''}" 
                           ...
                           <div class="pillow-subcategory-details">
                                 ${createPillowMaterialContentForItem(item, subcat, pillowData, productId, roomId, itemId, existingMaterials)}
                           </div>
                        </div>
                     `).join('')}
                  </div>
    ...
    function createPillowMaterialContentForItem(item, subcat, pillowData, productId, roomId, itemId, existingMaterials, index = 0) {
        const subcatData = pillowData[subcat.id];
        ...
        if (Object.keys(materialGroups).length > 0) {
            return Object.entries(materialGroups).map(([refLabel, materials]) => {
                return createPillowMaterialGroupForItem(item, subcat, refLabel, materials, allMaterials, productId, roomId, itemId, existingMaterials);
            }).join('');
        }
        // If we have active materials but no groups, create single input
        else if (activeMaterials.length > 0) {
            return createSinglePillowMaterialInputForItem(item, subcat, activeMaterials[0], allMaterials, productId, roomId, itemId, existingMaterials);
        }
        // If no active materials but have available materials, create empty input
        else if (allMaterials.length > 0) {
            return createSinglePillowMaterialInputForItem(item, subcat, null, allMaterials, productId, roomId, itemId, existingMaterials);
        }
        // No materials available
        else {
            return `
    ...
    function createPillowMaterialGroupForItem(item, subcat, refLabel, materials, allMaterials, productId, roomId, itemId, existingMaterials, index = 0) {
        const activeMaterial = materials && materials.length > 0 ? materials[0] : null;
        let existingMaterial = null;
        if (existingMaterials && existingMaterials.main && existingMaterials.main[index + 1] && existingMaterials.main[index + 1]['pillow']) {
            const pillowMaterials = existingMaterials.main[index + 1]['pillow'];
        ...
        const currentMaterial = existingMaterial || activeMaterial;
    ...
    function createSinglePillowMaterialInputForItem(item, subcat, activeMaterial, allMaterials, productId, roomId, itemId, existingMaterials, index = 0) {
        let existingMaterial = null;
        if (existingMaterials && existingMaterials.main && existingMaterials.main[index + 1] && existingMaterials.main[index + 1]['pillow']) {
            const pillowMaterials = existingMaterials.main[index + 1]['pillow'];
            for (const material of pillowMaterials) {
    ...
    $(document).on('input change', '.product-content .area-weight, .product-content .material-type-select, .product-content .pillow-quantity, .product-content .pillow-length, .product-content .pillow-width', function() {
        const $productContent = $(this).closest('.product-content');
        if ($productContent.length) {
            const productId = $productContent.attr('id').replace('product-', '').replace(/-room\d+$/, '');
            const roomId = $productContent.attr('id').match(/room(\d+)/)[1];
    ...
    $('#confirmQualificationSelect').on('click', function() {
        ...
        if (state.selectedQualification && state.currentRoom) {
            hideQualificationModal();
            if ([142, 181, 180, 178, 179, 183].includes(state.selectedQualification)) {
                $.post(ajax_url + '/api', {
                    get_product: 1,
                    ...
                    if (response.status === 'success') {
                        ...
                        if (selectedProductData) {
                            ...
                            setTimeout(() => {
                                calculateProductTotal(selectedProductData.product_id, state.currentRoom);
                            }, 2000);
                        } else {
...
<script>
    ...
    function collectProductDataForUpdate(productId, roomId, productType, availableIn, detailId = null) {
        ...
        if (availableIn === 'size') {
            ...
            if (selectedVariant) {
                productData.selected_variant = collectVariantDataForUpdate(productId, selectedVariant, roomId, productType);
            }
        }
        ...
        $(`#product-${productId}-room${roomId} .items-tab`).each(function() {
            ...
            if ($itemContent.length) {
                const itemData = {
                    ...
                    materials: collectMaterialsDataNewLayout(itemId, roomId, null, true),
    ...
    function collectVariantDataForUpdate(productId, variantId, roomId, productType) {
        ...
        // For curtain variants, add curtain data
        if (productType === 'curtain') {
            variantData.curtain_data = collectCurtainSetData($variant, productId, variantId, roomId);
        }
    ...
    function collectMaterialsDataNewLayout(productId, roomId, variantId = null, isItem = false) {
        ...
        $container.find('.material-tab-content').each(function() {
            ...
            let setLevel = 1;
            if (variantId) {
                const $variant = $container.closest('.product-variant-content');
                setLevel = $variant.index() + 1;
            }
            ...
            $materialTab.find('.material-group').each(function() {
                const $materialGroup = $(this);
                const refLabel = $materialGroup.data('ref-label') || 'A';
            ...
            $materialTab.find('.pillow-material-group').each(function() {
                const $pillowGroup = $(this);
                const refLabel = $pillowGroup.data('ref-label') || '1';

                const pillowData = {};

                // Collect pillow parts
                const faceMaterial = $pillowGroup.find('.material-type-select[data-subcategory="pillow_front"]').val();
                const backMaterial = $pillowGroup.find('.material-type-select[data-subcategory="pillow_back"]').val();
                const pipingMaterial = $pillowGroup.find('.material-type-select[data-subcategory="pillow_pipping"]').val();
                const defaultMaterial = $pillowGroup.find('.material-type-select[data-subcategory="default_pillow"]').val();

                if (faceMaterial || backMaterial || pipingMaterial) {
                    pillowData.face = faceMaterial || '';
                    pillowData.back = backMaterial || '';
                    pillowData.piping = pipingMaterial || '';
                } else if (defaultMaterial) {
                    pillowData.default = defaultMaterial;
                }

                if (Object.keys(pillowData).length > 0) {
                    if (!materials.main[setLevel]['pillow']) {
                        materials.main[setLevel]['pillow'] = [];
                    }

                    materials.main[setLevel]['pillow'].push([
                        refLabel,
                        pillowData
                    ]);
                }
    ...
    function mapCurtainMaterials(detail) {
        ...
        if (detail.curtain_data && Object.keys(detail.curtain_data).length > 0) {
            Object.entries(detail.curtain_data).forEach(([setIndex, setData]) => {
                const curtainData = setData.curtain1;
                ...
                materialsBySet[setIndex] = {
                    curtain: {
                        ...
                        height: curtainData.height,
                        unit_price: curtainData.unit_price,
                    ...
                    configuration: {
                        ...
                        motor_price: parseFloat(accessoryData.curtain_motor_price) || 0,
                        installation: accessoryData.curtain_installation,
                        installation_price: parseFloat(accessoryData.curtain_installation_price) || 0,
                        blackout_color: accessoryData.accessory_blackout_color || ''
                    },
                    accessories: accessoryData.accessory_types.map((type, index) => {
                        const accessoryId = accessoryData.accessory_ids[index];
                        return {
                            type: type,
                            id: accessoryId,
                            price: accessoryData.accessory_prices[accessoryId] || 0,
                            ...
                            total_price: accessoryData.accessory_total_prices[accessoryId] || 0
                        };
                    })
                };
            });
    ...
    function mapApiDataToExpectedFormat(detail) {
        const expectedData = {
            ...
            // product_type: detail.product_data?.type || 'product',
            // available_in: detail.product_data?.available_in || 'simple',
            // calculate_type: detail.calculate_type || 'standart'
        };
