               ...
               <div class="card-body">
                  <form id="add_order_form" enctype="multipart/form-data">
                     <div class="row">
                        ...
                        <div class="order-page-col col-md-3 col-sm-6">
                           <div class="form-group">
                              ...
                              <input type="text" name="order_delivery_date" class="form-control make-datepicker order_delivery_date required" id="lblOrderDeliveryDate" placeholder="<?php echo get_lang_text('orderadd_input_delivery_date'); ?> *" value="<?php echo date('d.m.Y'); ?>">
                     ...
                     <div class="form-group text-right">
                        <button type="submit" class="btn btn-success waves-effect waves-light btn-sb-form" id="add_order_btn">
...
<script>
   ...
   // Updated material cost calculation function for new pillow structure
   function calculateMaterialCosts($contentElement) {
      ...
      // Calculate pillow materials (new structure with labels and subcategories)
      $contentElement.find('.pillow-material-group .pillow-material-inputs-compact').each(function() {
         ...
         console.log('Pillow calculation for label', label, 'subcategory', subcategory, ':', {
            label: label,
            subcategory: subcategory,
            quantity: quantity,
            length: length,
            width: width,
            newMaterialPrice: newMaterialPrice,
            selectedMatPrice: selectedMatPrice,
            totalAdded: (newMatPrdPrice - selectedMatPrdPrice)
         });
      });
   ...
   function calculateAccessoryTotal($accessoryTab, productQuantity) {
      ...
      if (priceType === 'Per Piece') {
         ...
         const $multiplier = 0.2; // Default multiplier
...
<script>
   ...
   function createVariantsTabs(product, variants, roomId, productInstanceId) {
      return `
               ...
               <div class="product-variants-content" id="variants-content-${productInstanceId}-room${roomId}">
                  ${variants.map((variant, index) => `
                     <div class="product-variant-content ${index === 0 ? 'active' : ''}" 
                        ...
                        ${createVariantContentForSet(product, variant, roomId, productInstanceId)}
                     </div>
                  `).join('')}
               </div>
            </div>
                        ...
                        <div class="material-compact-fields">
                           ...
                           <div class="material-input${!isCurtainFabric?' d-none':''}">
                                 ...
                                 <input type="number" class="form-control curtain-fabric-length" 
                                       ...
                                       data-variant="${variantId}" data-category="${category}" data-ref-label="${refLabel}"
                                       value="${(activeMaterial && isCurtainFabric) ? activeMaterial[measurementField] || '' : ''}"
                                       step="0.01" min="0">
                           </div>
                           <div class="material-input${!isCurtainFabric?' d-none':''}">
                                 <label>Height</label>
   ...
   // Updated function to create pillow subcategories with vertical tabs
   function createPillowSubcategoriesForVariant(product, variant, roomId, activeMaterials, productInstanceId) {
      const variantId = product.available_in === 'size' ? variant.id : variant.product_id;
      ...
      // Define all possible pillow subcategories
      const subcategories = [{
            id: 'default',
            name: 'Default Pillow',
            icon: 'fa-pillow'
      ...
      // Filter to only show subcategories that have data
      const availableSubcategories = subcategories.filter(subcat =>
         pillowData[subcat.id] &&
         (pillowData[subcat.id].active || pillowData[subcat.id].all_materials)
      );
      ...
      if (availableSubcategories.length === 0) {
         return `
            <div class="no-materials-available">
                <i class="fa fa-info-circle"></i>
                <p>No pillow materials available</p>
            </div>
        `;
      }
                        ...
                        <div class="pillow-material-compact-fields">
                           ...
                           ${subcat.id === 'default' ? `
                                 ...
                                 <div class="pillow-material-input d-none">
                                    ...
                                    <div class="pillow-dimensions d-flex align-items-center d-none">
   ...
   function createCurtainVariantContent(product, variant, roomId, productInstanceId) {
      ...
      return `
            <div class="variant-details">
               <div class="compact-product-details">
                     ...
                     <div class="compact-details-fields">
                        ...
                        <div class="compact-detail-group">
                           ...
                           <textarea class="form-control product-notes-tr" 
                                 ...
                                 data-variant="${variant.product_id}"></textarea>
                        </div>
                        <div class="compact-detail-group">
                  ...
                  ${createMaterialSection(product, variant, roomId, productInstanceId)}
                        ...
                        <div class="curtain-control installation-control">
                           <div class="custom-control custom-checkbox">
                              ...
                              <label class="custom-control-label font-weight-bold" 
                                    ...
                                    <span class="installation-price text-success ml-2 font-weight-bold" style="display:none;">+ $200</span>
                              </label>
                           </div>
                        </div>
                     </div>
   ...
   function createVariantContentForSet(product, variant, roomId, productInstanceId) {
      // Check if this is a curtain product
      const isCurtainProduct = product.type === 'curtain';

      if (isCurtainProduct) {
         return createCurtainVariantContent(product, variant, roomId, productInstanceId);
      } else {
         const dims = variant.dims || {}; // fallback if undefined
                     ...
                     <div class="compact-product-details">
                        ...
                        <div class="compact-details-with-image">
                              ...
                              <div class="compact-details-fields">
                                 <div class="compact-detail-group">
                                    <label>Width (m)</label>
                                    <input type="number" class="form-control dimension-width" data-standart_width="${width}" 
                                          value="${width}" placeholder="0.00" step="0.01" min="0" 
                                          data-variant="${variant.product_id}" ${getDisabledAttr(variant.calculate_type, 'w')}>
                                 </div>
                                 <div class="compact-detail-group">
                                    <label>Length (m)</label>
                                    <input type="number" class="form-control dimension-length" data-standart_length="${length}" 
                                 ...
                                 <div class="compact-detail-group">
                                    ...
                                    <input type="number" class="form-control dimension-height" data-standart_height="${height}" 
   ...
   function createVariantContentForSize(product, variant, roomId, productInstanceId) {
                     ...
                     <div class="compact-details-with-image">
                        ...
                        <div class="compact-details-fields">
                              ...
                              <div class="compact-detail-group">
                                 ...
                                 <textarea class="form-control product-notes-tr" 
                                    placeholder="Enter notes in Turkish"
                                    data-variant="${variant.id}"></textarea>
                              </div>
                              <div class="compact-detail-group">
                                 <label>Notes(EN)</label>
                                 <textarea class="form-control product-notes-en" 
   ...
   // Update the setupMaterialTabsForProduct function to handle nested pillow tabs
   ...
   // Function to setup label tabs for ALL material categories
   ...
   // Function to setup label tabs for a specific material category
            ...
            filteredQualifications.forEach((qual, index) => {
               const $option = $(`
                     <div class="qualification-option" data-qualification="${qual.attr_ids}" data-product-names="${qual.product_names}">
                        <div class="qualification-select-option-image">
                           <img src="<?= URL ?>/uploads/online-img/${qual.online_product_img}" alt="${qual.attr_name}">
                        </div>
               ...
               filteredProducts.forEach(product => {
                  ...
                  const $option = $(`
                     <div class="multi-select-option" 
                           ...
                           data-attr-id="${product.attr_id}" 
                           ${search_item_of === 'plan_products' ? `onclick="getItem_DetailForCombo(${product.other_data['order_id']}, ${product.other_data['detail_id']})"` : ''}>                                 <small class="brand-tag" style="background: #6c757d; color: white; padding: 2px 6px; border-radius: 3px; font-size: 0.7rem;position:absolute;top:0;right:0;">
                           ...
                           <div class="accessory-selection-container">
                              <select class="form-control accessory-options-select">
                                 ...
                                 ${accessoryOptions.map(option => `
                                    <option value="${option.accessory_id}" data-accessory-type="${accessory.id}" data-price="${option.price}" data-price-type="${option.price_type}" data-price-depends-on="${option.price_depends_on}" data-image="${option.accessory_img}">${option.accessory_name}</option>
                                 `).join('')}
                              </select>
                           </div>
                        </div>
   ...
   function addProductTab(roomId, product) {
      ...
      const $tab = $(`
            <div class="product-tab" data-product="${productInstanceId}" data-type="${product.type}" data-available-in="${product.available_in}" id="${tabId}">
      ...
      setTimeout(() => {
         loadProductContent(contentId, product, roomId, productInstanceId);
      }, 100);
      setTimeout(() => {
         calculateProductTotal(productInstanceId, state.currentRoom);
      }, 1500);
   }
   ...
   // Update your loadProductContent function to ensure proper initialization
   function loadProductContent(contentId, product, roomId, productInstanceId) {
      ...
      if (product.has_variant && product.available_in === 'set') {
         $.post(ajax_url + '/api', {
            ...
            if (response.status === 'success') {
               ...
               loadProductWithVariants($content, product, variants, roomId, productInstanceId);
         ...
         if (product.type === 'fitout') {
            loadFitoutProductContent($content, product, roomId, productInstanceId);
         } else if (product.type === 'curtain') {
            loadCurtainProductContent($content, product, product, roomId, productInstanceId);
         } else {
            loadSimpleProductContent($content, product, product, roomId, productInstanceId);
         }

         // Setup material tabs with proper delay
         setTimeout(() => {
   ...
   function loadProductWithVariants($content, product, variants, roomId, productInstanceId) {
      ...
      if (product.available_in == 'set') {
         ...
         variantsHTML = createVariantsTabs(product, variants, roomId, productInstanceId);
      } else {
         variantsHTML = createVariantsRadioSelection(product, variants, roomId, productInstanceId);
      }
      ...
      // In the loadProductContent function, update the setTimeout for setup
      setTimeout(() => {
         if (product.has_variant && product.available_in === 'set') {
   ...
   function createVariantsRadioSelection(product, variants, roomId, productInstanceId) {
               ...
               <div class="product-variants-content" id="variants-content-${productId}-room${roomId}">
                  ${variants.map((variant, index) => `
                     <div class="product-variant-content ${index === 0 ? 'active' : ''}" 
                        ...
                        ${createVariantContentForSize(product, variant, roomId, productId)}
                     </div>
                  `).join('')}
               </div>
            </div>
   ...
   function loadFitoutProductContent($content, product, roomId, productInstanceId) {
   ...
   function loadSimpleProductContent($content, product, variant, roomId, productInstanceId) {
                     ...
                     <div class="compact-details-with-image">
                        ...
                        <div class="compact-details-fields">
                              ...
                              <div class="compact-detail-group">
                                 <label>Notes(TR)</label>
                                 <textarea class="form-control product-notes-tr" 
                                    placeholder="Enter notes in Turkish"></textarea>
                              </div>
                              <div class="compact-detail-group">
                                 <label>Notes(EN)</label>
                                 <textarea class="form-control product-notes-en" 
                                    placeholder="Enter notes in English"></textarea>
                              </div>
                        </div>
                     </div>
                     <!-- Add base price field for variant -->
   ...
   function loadCurtainProductContent($content, product, variant, roomId, productInstanceId) {
      const dims = product.dims || {};
      const unit_price = calculateUnitPrice(product.calculate_type, dims);
   ...
   function createMaterialSection(product, variant, roomId, productInstanceId) {
   ...
   // Complete function to create material content with label tabs for ALL categories
   function createMaterialContentWithLabels(product, variant, category, roomId, activeMaterials, productInstanceId) {
      ...
      if (category.id === 'pillow') {
         return createPillowStructure(product, variant, categoryData, roomId, productInstanceId, variantId);
      }
            ...
            <div class="material-label-content" id="${category.id}LabelContent-${productInstanceId}-${variantId}-room${roomId}">
                ${labelTabs.map((labelTab, index) => `
                    <div class="material-label-tab-content ${index === 0 ? 'active' : ''}" 
                       ...
                       ${createMaterialLabelContent(product, variant, category.id, labelTab.label, categoryData, roomId, productInstanceId, variantId)}
                    </div>
                `).join('')}
            </div>
        </div>
   ...
   // Special function for pillow structure (with subcategories inside each label)
   function createPillowStructure(product, variant, pillowData, roomId, productInstanceId, variantId) {
                                 ...
                                 <div class="pillow-subcategories-content" id="pillowSubContent-${productInstanceId}-${variantId}-room${roomId}-${labelTab.label}">
                                    ${pillowSubcategories.map((subcat, subIndex) => `
                                       <div class="pillow-subcategory-content ${subIndex === 0 ? 'active' : ''}" 
                                          ...
                                          <div class="pillow-subcategory-details">
                                                ${createPillowSubcategoryContent(product, variant, subcat, labelTab.label, pillowData, roomId, productInstanceId, variantId)}
                                          </div>
                                       </div>
                                    `).join('')}
                                 </div>
   ...
   // Function to create content for a specific label within a material category (non-pillow)
   function createMaterialLabelContent(product, variant, category, label, categoryData, roomId, productInstanceId, variantId) {
      const materialGroups = categoryData.materialGroups || {};
      const activeMaterialsList = categoryData.active || [];
      ...
      if (materialsForLabel.length === 0 && allMaterials.length > 0) {
         ...
         return createSingleMaterialInputForLabel(product, variant, category, label, null, allMaterials, roomId, variantId);
      }
   ...
   // Function for single material input for a specific label (non-pillow)
   function createSingleMaterialInputForLabel(product, variant, category, label, activeMaterial, allMaterials, roomId, variantId) {
      const standardMaterialPrice = allMaterials.find(mat => mat.material_id == activeMaterial.material_id)?.material_price || 0;
   ...
   // Updated function to create pillow content with labels (1, 2, 3, etc.)
   function createPillowContentWithLabels(product, variant, pillowData, roomId, productInstanceId, variantId) {
      // Since pillow is now handled by createMaterialContentWithLabels, 
      // we just need to ensure it creates the special pillow structure
      const category = {
         id: 'pillow',
         name: 'Pillow',
         active_materials: pillowData
      };

      // Call the main function but with pillow-specific handling
      return createMaterialContentWithLabels(product, variant, category, roomId, {
         pillow: pillowData
      }, productInstanceId);
   }

   // Function to create content for a specific pillow subcategory within a label
   function createPillowSubcategoryContent(product, variant, subcat, label, pillowData, roomId, productInstanceId, variantId) {
      const subcatData = pillowData[subcat.id];
      ...
      if (materialsForLabel.length === 0 && allMaterials.length > 0) {
         // No specific materials for this label, show empty selection
         return createSinglePillowMaterialInputForLabel(product, variant, subcat, label, null, allMaterials, roomId, variantId);
      }
   ...
   // Function for single pillow material input for a specific label
   function createSinglePillowMaterialInputForLabel(product, variant, subcat, label, activeMaterial, allMaterials, roomId, variantId) {
      const standardMaterialPrice = allMaterials.find(mat => mat.material_id == activeMaterial.material_id)?.material_price || 0;
   ...
   // Helper function to get pillow type display name
   function getPillowTypeName(type) {
      const typeNames = {
         'default': 'Default Pillow',
         'front': 'Pillow Front',
         'back': 'Pillow Back',
         'pipping': 'Pillow Pipping',
         'piping': 'Pillow Piping'
      };
      return typeNames[type] || type.charAt(0).toUpperCase() + type.slice(1);
   }

   // Function to create content for a specific pillow type
   function createPillowTypeContent(product, variant, pillowType, pillowTypeData, roomId, productInstanceId, variantId) {
      if (!pillowTypeData) {
         return `
            <div class="no-materials-available">
                <i class="fa fa-info-circle"></i>
                <p>No materials available for ${getPillowTypeName(pillowType)}</p>
            </div>
        `;
      }

      const materialGroups = pillowTypeData.materialGroups || {};
      const activeMaterials = pillowTypeData.active || [];
      const allMaterials = pillowTypeData.all_materials || [];

      console.log('Creating pillow type content:', {
         pillowType: pillowType,
         materialGroups: materialGroups,
         activeMaterials: activeMaterials
      });

      // If we have material groups (A, B, C, etc.)
      if (Object.keys(materialGroups).length > 0) {
         return Object.entries(materialGroups).map(([refLabel, materials]) => {
            const activeMaterial = materials && materials.length > 0 ? materials[0] : null;
            const standardMaterialPrice = allMaterials.find(mat => mat.material_id == activeMaterial.material_id)?.material_price || 0;
            ...
            return `
                <div class="pillow-material-group" data-ref-label="${refLabel}" data-pillow-type="${pillowType}" data-mt_of="${pillowType}">
                    <div class="pillow-material-group-header">
                        <h6>Material ${refLabel} ${activeMaterial?.alias_name ? `- ${activeMaterial.alias_name}` : ''}</h6>
                    </div>
                    <div class="material-grid">
                        <div class="pillow-material-inputs-compact" data-mt_of="${pillowType}">
                            <div class="pillow-material-compact-image">
                                ${activeMaterial?.material_img ? 
                                    `<img src="<?= URL ?>/uploads/material/${activeMaterial.material_img}" alt="${activeMaterial.material_name}" style="width:100%;height:100%;object-fit:cover;">` : 
                                    `<i class="fa fa-image"></i>`
                            ...
                            <div class="pillow-material-compact-fields">
                                <div class="pillow-material-input">
                                    <label>Material</label>
                                    <select class="form-control make-it-select material-type-select" 
                                          data-pillow-type="${pillowType}" data-variant="${variantId}" data-ref-label="${refLabel}">
                                        <option value="">Select Material</option>
                                        ${allMaterials.map(material => `
                                            <option value="${material.material_id}" 
                                                ${activeMaterial && activeMaterial.material_id == material.material_id ? 'selected' : ''}
      ...
      // If we have active materials but no groups
      else if (activeMaterials.length > 0) {
         return createSinglePillowMaterialInput(product, variant, {
            id: pillowType,
            name: getPillowTypeName(pillowType)
         }, activeMaterials[0], allMaterials, roomId, variantId);
      }
      // If no active materials but have available materials
      else if (allMaterials.length > 0) {
         return createSinglePillowMaterialInput(product, variant, {
            id: pillowType,
            name: getPillowTypeName(pillowType)
         }, null, allMaterials, roomId, variantId);
      }
      // No materials available
      else {
         return `
   ...
   function updatePillowSubcategoryStatus(productId, variantId, availableIn, subcategoryId, roomId, type) {
      // We need to find which label tab is active first
      const $activeLabelTab = $(`#pillowLabelTabs-${productId}-${variantId}-room${roomId} .material-label-tab.active`);
      if (!$activeLabelTab.length) return;

      const label = $activeLabelTab.data('label');
      const $pillowTabs = $(`#pillowSubTabs-${productId}-${variantId}-room${roomId}-${label}`);

      if (!$pillowTabs.length) return;

      const $tab = $pillowTabs.find(`.pillow-subcategory-tab[data-subcategory="${subcategoryId}"]`);
      const $statusIndicator = $tab.find('.status-indicator');

      const $content = $(`#pillowSubcategory-${productId}-${variantId}-room${roomId}-${label}-${subcategoryId}`);
   ...
   $(document).on('input change', '.product-content .dimension-width, .product-content .dimension-length, .product-content .dimension-height, .product-content .product-qty, .product-content .product-discount, .product-content .surcharge-checkbox', function() {
      ...
      console.log('I product-qty:', $this.hasClass('product-qty'));
      if (!$productContent.length) return;

      const productId = $productContent.attr('id').replace('product-', '').replace(/-room\d+$/, '');
      const roomId = $productContent.attr('id').match(/room(\d+)/)[1];

      // If it's a quantity input, handle stock validation and delivery date first
      if ($this.hasClass('product-qty')) {
         console.log('auto set delivery date...');
            ...
            if (product_count > stock_product_count) {
               ...
               swal({
                  ...
                  if (isConfirm) {
                     ...
                     // Trigger calculation after quantity adjustment
                     calculateProductTotal(productId, roomId);
                  } else {
                     $inputElement.val("");
                  }
         ...
         if ($("#date_modified").val() == "0") {
            ...
            console.log('product_shipping_data:', product_shipping_data);
      ...
      // Use debouncing for calculations (for all input types including quantity)
      debounce(() => {
         calculateProductTotal(productId, roomId);
      }, 300)();
   });
      ...
      if (state.selectedAccessory && state.selectedAccessoryAttrId) {
         ...
         if (state.currentVariantId && state.currentProductId && state.currentRoom) {
            ...
            $.post(ajax_url + '/api', {
               ...
               // console.log(data);
               var response = $.parseJSON(data);
               console.log('selected accessory:', response);
               if (response.status === 'success') {
                  // Find the selected product
                  const accessoryOptions = response.data;
...
<script>
   ...
   // Function to collect product data with new structure
   function collectProductData(productId, roomId, productType, availableIn) {
      ...
      if (availableIn === 'set') {
         productData.sets = collectSetsData(productId, roomId, productType);
      }
      ...
      // Collect materials with proper structure for detail_attr
      ...
      $(`#product-${productId}-room${roomId} .items-tab`).each(function() {
         ...
         if ($itemContent.length) {
            const itemData = {
               ...
               materials: collectMaterialsDataNewLayout(itemId, roomId, null, itemId),
   ...
   // Function to collect surcharges data for attr_rates
   ...
   // Improved function to collect materials data with proper pillow structure
      ...
      $(`[id^="materialContent-${prefix}-room${roomId}-"]`).each(function() {
         ...
         $categoryContent.find('.material-group').each(function() {
            ...
            if (materialId) {
               ...
               const standardPrice = parseFloat($selectedOption.data('standard-material-price')) || 0;
               const areaWeight = parseFloat($materialGroup.find('.area-weight').val()) || 0;
   ...
   // Function to collect sets/variants data for detail_attr
   function collectSetsData(productId, roomId, productType) {
      const sets = [];
   ...
   function collectSizeData(productId, variantId, roomId) {
   ...
   // the materials collection function to handle items
   function collectMaterialsData(productId, roomId, variantId = null, isItem = false) {
      ...
      $(`#${prefix}-room${roomId} .pillow-material-group`).each(function() {
         ...
         const faceMaterial = $pillow.find('.material-type-select[data-subcategory="front"]').val();
         if (faceMaterial) {
            materials.push({
               material_id: faceMaterial,
               area_weight: 1, // Default for pillows
               material_price: parseFloat($pillow.find('.material-type-select[data-subcategory="front"] option:selected').data('price')) || 0,
               category: 'pillow',
               ref_label: refLabel,
               subcategory: 'face'
            });
         ...
         const backMaterial = $pillow.find('.material-type-select[data-subcategory="back"]').val();
         if (backMaterial) {
            materials.push({
               material_id: backMaterial,
               area_weight: 1, // Default for pillows
               material_price: parseFloat($pillow.find('.material-type-select[data-subcategory="back"] option:selected').data('price')) || 0,
               category: 'pillow',
               ref_label: refLabel,
               subcategory: 'back'
            });
   ...
   // Function to submit the order with new layout
   function submitNewOrder() {
      ...
      // Show loading state
      $('.btn-sb-form').prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> Processing...');
      ...
      formData.append('add_order_with_newlayout', '1');
      ...
      $.ajax({
         ...
         success: function(data) {
            ...
            if (response.status === "success") {
               $('.btn-sb-form').prop('disabled', false).html('Finish Order');
               alert('Order created successfully!');
               // window.location.href = '<?= URL ?>/index.php?page=orders';
            } else {
               alert('Error: ' + response.message);
               $('.btn-sb-form').prop('disabled', false).html('Create Order');
            }
         },
