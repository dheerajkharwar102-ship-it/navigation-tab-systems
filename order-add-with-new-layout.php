            ...
            <div class="card m-b-30">
               <div class="card-body">
                  <form id="add_order_form" enctype="multipart/form-data">
                     <div class="row">
                        <div class="order-page-col col-md-3 col-sm-6 d-none">
                        ...
                        <div class="order-page-col col-md-4 col-sm-6 full-w-s">
                           ...
                           <div class="form-group show-extra-agreement-row row" style="display: none;">
                              <label for="" class="col-sm-12"><?php echo get_lang_text('orderadd_input_contract_extra'); ?></label>
                              <div class="col-sm-6">
                        ...
                        <div class="order-page-col col-md-4 col-sm-6 full-w-s d-none">
                           <div class="form-group">
                     ...
                     <div class="qualification-modal" id="mainCategoryModal">
                        <div class="qualification-modal-content">
                           ...
                           <div class="qualification-modal-body">
                              ...
                              <div class="qualification-options" id="catQualificationOptions"></div>
...
<script>
   ...
   const filterState = {
      product: {
         selectedBrand: '1', // 0 means "All Brands"
         selectedStyles: [],
         minPrice: 0,
         maxPrice: 50000,
         dateSort: 'newest',
            ...
            filteredQualifications.forEach((qual, index) => {
               ...
               const $option = $(`
                     <div class="qualification-option${display_class}" data-qualification="${qual.attr_ids}" data-product-names="${qual.product_names}">
                        <div class="qualification-select-option-image">
                           <img src="<?= URL ?>/uploads/online-img/${qual.icon}" alt="${qual.attr_name}" onerror="this.onerror=null; this.src='<?= URL ?>/uploads/${qual.icon}'">
                        </div>
   ...
   function filterMainCategoryOptions(searchTerm) {
      ...
      return new Promise((resolve) => {
         const $options = $('#mainCategoryOptions .qualification-option');
         $options.hide();
   ...
   function filterQualificationOptions(searchTerm, applySearchFilterOnProduct = true, notSearchTerm = '') {
      $('#qualificationOptions .qualification-option').removeClass('selected');
      ...
      $options.each(function() {
         ...
         if (name.includes(searchLower) || (applySearchFilterOnProduct && product_names.includes(searchLower))) {
            // If notSearchTerm is not empty, exclude items containing it
            if (notSearchTerm && notSearchTerm !== '') {
               // Only show if name does NOT contain notSearchTerm
               if (!name.includes(notSearchTerm)) {
                  $option.removeClass('d-none').show();
                  hasMatches = true;
               }
               // If name contains notSearchTerm, don't show (it stays hidden)
            } else {
               // If notSearchTerm is empty or falsy, show all matches
               $option.removeClass('d-none').show();
               hasMatches = true;
            }
         }
      ...
      $.post(ajax_url + '/api', {
         ...
         try {
            ...
            if (response.status === 'success') {
               ...
               //                   <div class="multi-select-options" id="multiSelectOptions"></div>
               //                   <div class="load-more-div text-center mt-3">
               //                      <button type="button" class="btn btn-outline-primary" id="loadMoreProductsInCatBtn">
               //                         <i class="fa fa-sync-alt mr-1"></i> Load More Products
               //                      </button>
               //                   </div>
               //                </div>
               //             `;
               // $targetDiv.append($loadMoreBtn);
            }
         } catch (error) {
   ...
   function loadQualificationOptionsToDiv(searchTerm, targetDivId) {
      return new Promise((resolve) => {
         const $targetDiv = $(`#${targetDivId}`);
         ...
         // If search term is empty, clear product results and return
         if (!searchTerm || searchTerm.trim() === '') {
            $targetDiv.empty(); // Use empty() instead of html('')
            resolve();
            return; // Add return here to stop execution
         }
         ...
         $.post(ajax_url + '/api', {
            ...
            if (response.status === 'success') {
               ...
               filteredQualifications.forEach((qual, index) => {
                  ...
                  const $option = $(`
                  <div class="qualification-option${display_class}" 
                       data-qualification="${qual.attr_ids}" 
                       data-product-names="${qual.product_names}">
                     <div class="qualification-select-option-image">
                        <img src="<?= URL ?>/uploads/online-img/${qual.icon}" alt="${qual.attr_name}" onerror="this.onerror=null; this.src='<?= URL ?>/uploads/${qual.icon}'">
                     </div>
                     <div class="qualification-option-name">${qual.attr_name}</div>
               ...
               `);
                  ...
                  $('#qualificationOptions').append($option.clone());
                  $targetDiv.append($option);
               });

               state.selectedQualification = null;
               $(`#${targetDivId} .qualification-option`).removeClass('selected');
               $(`#${targetDivId} .qualification-option`).show();

               console.log(`Added ${filteredQualifications.length} qualifications to #${targetDivId}`);
               resolve();
            } else {
               // Handle error response
               $targetDiv.html(`<div class="error-message">Error: ${response.message || 'Failed to load qualifications'}</div>`);
               console.error('API Error:', response);
               resolve();
            }
         }).fail(function(jqXHR, textStatus, errorThrown) {
            // Handle AJAX failure
            $targetDiv.html(`<div class="error-message">AJAX Error: ${textStatus}</div>`);
   ...
   $('#mainCategorySearch').on('input', function() {
      ...
      filterTimeout = setTimeout(() => {
         // 300ms delay
         filterMainCategoryOptions(searchTerm).then(() => {
            loadQualificationOptionsToDiv(searchTerm.trim(), 'catQualificationOptions').then(() => {
               setTimeout(() => {
   ...
   $(document).on('click', '#catQualificationOptions .qualification-option', function() {
      console.log('Qualification option clicked:', $(this).data('qualification'));
      $('.qualification-option').removeClass('selected');
      $(this).addClass('selected');
      state.selectedQualification = $(this).data('qualification');
   ...
   $('#confirmQualificationSelect').on('click', function() {
      ...
      if (state.selectedQualification && state.currentRoom) {
         if (String(state.selectedQualification || '').split('_').some(id => [142, 181, 180, 178, 179, 183].includes(Number(id)))) {
            hideQualificationModal();
            hideMainCategoryModal();
            console.log('Selected Curtain Qualificatin');
            $.post(ajax_url + '/api', {
               get_product: 1,
            ...
            hideMainCategoryModal();
            $('#qualificationModal').fadeIn(300);
            filterQualificationOptions('sofa', false, 'sofas');
            sofasubmenu = true;
         } else {
            hideQualificationModal();
            hideMainCategoryModal();
...
<script>
   ...
   function collectProductData(productId, roomId, productType, availableIn) {
      ...
      // Collect design images for all products
      let design_imgs = [];
      $productContent.find('.product-image-input').each(function() {
         const file = this.files[0];
         if (file) {
            design_imgs.push({
               file: file,
      ...
      // If no files selected, check if there are existing images
      if (design_imgs.length === 0) {
         $productContent.find('.product-image-preview img').each(function() {
      ...
      productData.design_img_data = design_imgs;
   ...
   function collectSetsData(productId, roomId, productType) {
      ...
      $(`#variants-content-${productId}-room${roomId} .product-variant-content`).each(function() {
         ...
         const setData = {
            ...
            materials: collectMaterialsDataNewLayout(productId, roomId, variantId, null, $index),
