               ...
               <div class="product-variants-content" id="variants-content-${productId}-room${roomId}">
                  ${variants.map((variant, index) => `
                     <div class="product-variant-content ${index === 0 ? 'active' : ''}" 
                        ...
                        ${createVariantContentForSize(product, variant, roomId, productInstanceId)}
                     </div>
                  `).join('')}
               </div>
            </div>
...
<script>
   ...
   function collectProductData(productId, roomId, productType, availableIn) {
      ...
      if (availableIn === 'size') {
         ...
         if (selectedVariant) {
            productData.selected_variant = collectVariantData(productId, selectedVariant, roomId, productType);
         }
      }
   ...
   function collectMaterialsDataNewLayout(productId, roomId, variantId = null, itemId = null) {
      ...
      $container.find('.material-tab-content').each(function() {
         ...
         $materialTab.find('.pillow-material-group').each(function() {
            ...
            const faceMaterialReplacement = $pillowGroup.find('.material-replacement[data-subcategory="front"]').val();
            const backMaterialReplacement = $pillowGroup.find('.material-replacement[data-subcategory="back"]').val();
            const pipingMaterialReplacement = $pillowGroup.find('.material-replacement[data-subcategory="pipping"]').val();
            const defaultMaterialReplacement = $pillowGroup.find('.material-replacement[data-subcategory="default"]').val();
            ...
            const replacement = faceMaterialReplacement || backMaterialReplacement || pipingMaterialReplacement || defaultMaterialReplacement || '';
