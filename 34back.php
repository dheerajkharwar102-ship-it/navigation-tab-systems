...
if (isset($_POST['get_materials_by_category'])) {
    ...
    $standard_material_price = $_POST['standard_material_price'] ?? '';
    $replacement_category = $_POST['replacement_category'] ?? '';
    if ($replacement_category != '') {
        return getAllMaterialsByCategory($mt, $replacement_category);
    }
    $materials = getAllMaterialsByCategory($mt, $category);

    $options_html = '';
    foreach ($materials as $material) {
        $isSelected = $selected_material == $material->material_id ? 'selected' : '';
        $options_html .= "<option value='{$material->material_id}' 
                                {$isSelected}
                                data-price='{$material->material_price}'
                                data-standard-material-price='{$standard_material_price}'
