...
<script>
    ...
    // dimension change handlers with proper product type detection
    $(document).on('input change', '.dimension-width, .dimension-length, .dimension-height, .product-qty, .product-discount, .surcharge-checkbox', function() {
        const $productContent = $(this).closest('.product-content, .variant-details');
        if ($productContent.length) {
            const productId = $productContent.closest('[id*="product-"]').attr('id').replace('product-', '').replace(/-room\d+$/, '');
            const roomId = $productContent.closest('[id*="room"]').attr('id').match(/room(\d+)/)[1];

            // Let the function detect the product type automatically
            calculateProductTotal(productId, roomId);
        }
    });

    // Enhanced material change handlers
