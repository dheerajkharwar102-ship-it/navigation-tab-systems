    ...
    if ($insert) {
        ...
        foreach ($order_details as $detail_index => $detail) {
            ...
            if ($detail_id) {
                ...
                // Save surcharges/rates
                if (isset($detail['surcharges']) && is_array($detail['surcharges'])) {
                    saveOrderSurchargesNewLayout($o, $order_id, $detail_id, $detail['surcharges']);
                }
            }
        }
...
function processProductDataNewLayout($productData, $roomInfo, $product_order_no)
{
    ...
    if (isset($productData['materials']) && is_array($productData['materials'])) {
        $material_result = calculateMaterialCostsNewLayout($productData['materials'], $product_attr, $productData);
        $material_cost = $material_result['cost'];
        $selected_materials = $material_result['materials'];
    }
    ...
    if (isset($productData['surcharges']) && is_array($productData['surcharges'])) {
        $surcharge_result = calculateSurchargesNewLayout($productData['surcharges'], $base_price + $material_cost, $product_attr);
    ...
    $attr_data = prepareProductDimensionsNewLayout($product, $productData, $product_attr, $detail_attr_rates);
...
function prepareProductDimensionsNewLayout($product, $productData, $productAttr, $detail_attr_rates)
{
    $attr_data = [
        'attr_bed_dim' => 0,
        'attr_dims' => '',
    ...
    if ($productAttr->attr_type == 'bed') {
        ...
        if (isset($bed_dims[$bed_dim])) {
            $dims_data[] = [
                'width' => $bed_dims[$bed_dim]['width'] ?? 0,
                'length' => $bed_dims[$bed_dim]['length'] ?? 0,
                'height' => $bed_dims[$bed_dim]['height'] ?? 0,
                'standart_width' => $bed_dims['180200']['width'] ?? 0,
                'standart_length' => $bed_dims['180200']['length'] ?? 0,
                'standart_height' => $bed_dims['180200']['height'] ?? 0,
                'standart_price' => $product->standart_price ?? 0,
                'unit_price' => calculateBedUnitPrice($product, $bed_dim),
                'bed_dim' => $bed_dim
            ];
        ...
        if (isset($productData['sets']) && is_array($productData['sets'])) {
            foreach ($productData['sets'] as $setData) {
                ...
                $dims_data[] = [
                    ...
                    'calculate_type' => $setData['calculate_type'] ?? 'standart'
                ];
            }
        } else {
            ...
            $dims_data[] = [
                ...
                'quantity' => floatval($productData['quantity'] ?? 1),
                'unit_price' => floatval($productData['unit_price'] ?? 0),
                'calculate_type' => $productData['calculate_type'] ?? 'standart'
            ];
        }
    }
...
function calculateMaterialCostsNewLayout($materials, $productAttr, $productData)
{
    $m = new Material();
    $total_cost = 0;
    $selected_materials = [];
    ...
    if (isset($materials['main']) && is_array($materials['main'])) {
        foreach ($materials['main'] as $set_level => $categories) {
            foreach ($categories as $category => $material_groups) {
                foreach ($material_groups as $material_info) {
                    if (is_array($material_info) && count($material_info) >= 2) {
                        $material_id = $material_info[1]; // Second element is material_id
                        if (!empty($material_id)) {
                            $material = $m->getMaterial($material_id);
                            if ($material) {
                                $material_cost = calculateSingleMaterialCostNewLayout($material, $category, $material_info, $productData);
                                ...
                                $selected_materials[] = [
                                    ...
                                    'cost' => $material_cost,
                                    'ref_label' => $material_info[0] ?? 'A',
                                    'replacement' => $material_info[2] ?? ''
                                ];
                            }
                        }
                    }
...
function calculateSingleMaterialCostNewLayout($material, $category, $material_info, $productData)
{
...
function calculateSurchargesNewLayout($surcharges, $base_amount, $productAttr)
{
    $total_surcharge = 0;
    $rates_data = [];
...
function saveOrderDetailNewLayout($o, $orderId, $detail, $row_number)
{
    ...
    if ($detail_id) {
        // Save attribute data (detail_attr)
        $attr_data = $detail['detail_attr'];
        $insert_detail_attr = $o->addOrderDetailAttr($attr_data, $orderId, $detail_id);
...
function saveOrderMaterialsWithSets($o, $orderId, $detailId, $selected_product_images)
{
    foreach ($selected_product_images as $set_level => $image_data) {
        if (isset($image_data['main']) && is_array($image_data['main'])) {
            foreach ($image_data['main'] as $material_type => $materials) {
                foreach ($materials as $material_info) {
                    if ($material_type == 'pillow') {
                        $label = $material_info[0];
                        $pillow_data = $material_info[1];

                        if (is_array($pillow_data)) {
                            foreach ($pillow_data as $pillow_part => $pillow_material_id) {
                                if (!empty($pillow_material_id)) {
                                    $data_pillow = [
                                        "order_id" => $orderId,
                                        "detail_id" => $detailId,
                                        "set_id" => $set_level,
                                        "ref_label" => $label,
                                        "material_id" => $pillow_material_id,
                                        "image_type" => 'pillow',
                                        "mt_type" => 'main',
                                        "pillow_mt" => $pillow_part,
                                    ];
                        ...
                        list($label, $img_id, $replace_mt) = $material_info;
                        if (!empty($img_id)) {
                            $data = [
                                "order_id" => $orderId,
                                "detail_id" => $detailId,
                                ...
                                "ref_label" => $label,
                                "material_id" => $img_id,
                                "image_type" => $material_type,
                                "mt_type" => 'main',
                                "replacement" => $replace_mt ?? '',
                            ];
        ...
        // Handle optional materials if needed
        if (isset($image_data['optional']) && is_array($image_data['optional'])) {
            foreach ($image_data['optional'] as $material_type => $materials) {
                foreach ($materials as $material_info) {
                    list($label, $img_id) = $material_info;
                    if (!empty($img_id)) {
                        $data = [
                            "order_id" => $orderId,
                            "detail_id" => $detailId,
...
if (isset($_POST['update_order_with_newlayout'])) {
    ...
    if ($update) {
        ...
        foreach ($order_details as $detail_index => $detail) {
            $detail_id = saveOrderDetailNewLayout($o, $order_id, $detail, $detail_index + 1);
            if ($detail_id) {
                // Update or add to planning
                $plan->deletePlanningFabricInfo($order_id, $detail_id);
                $add_plan_data = $plan->addPlan($order_id, $detail_id);
...
// Add these missing material saving functions
function saveOrderMaterialsNewLayout($o, $orderId, $detailId, $materials)
{
    foreach ($materials as $material) {
        $material_data = [
            ...
            'material_type' => $material['category'] ?? 'main',
            'set_level' => $material['set_level'] ?? 1,
            'ref_label' => $material['ref_label'] ?? 'A',
            'cost' => $material['cost'] ?? 0,
            'replacement' => $material['replacement'] ?? ''
        ];

        $o->addOrderDetailMaterial($material_data);
    }
}

function saveOrderSurchargesNewLayout($o, $orderId, $detailId, $surcharges)
{
    foreach ($surcharges as $surcharge) {
        $surcharge_data = [
            'order_id' => $orderId,
            ...
            'surcharge_type' => $surcharge['type'] ?? 'plus',
            'surcharge_rate' => floatval($surcharge['rate'] ?? 0),
            'surcharge_amount' => floatval($surcharge['amount'] ?? 0)
        ];

        $o->addOrderDetailSurcharge($surcharge_data);
    }
}
