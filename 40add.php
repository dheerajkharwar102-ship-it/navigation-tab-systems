...
<script>
         ...
         try {
            ...
            if (response.status === 'success') {
               ...
               products.forEach(product => {
                  const other_buttons = searchType !== 'special' ? getOtherRelatedButtons(product, 'no') : ['', ''];
   ...
   function filterQualificationOptions(searchTerm) {
      ...
      $options.each(function() {
         ...
         if (name.includes(searchLower) || product_names.includes(searchLower)) {
            $option.show();
            hasMatches = true;
         }
      });
   ...
   function getOtherRelatedButtons(item, isProductModalVisible = 'yes') {
   ...
   function createMaterialSectionForItem(item, productId, roomId) {
      ...
      const product = {
         ...
         available_in: item.available_in || 'size',
         type: item.type || 'standard',
         id: item.id || itemId
      };

      // Create variant object from item data
      const variant = {
         id: itemId,
         product_id: itemId,
         product_name: item.product_name || itemName
      ...
      // Generate productInstanceId
      const productInstanceId = `${itemId}-${productId}-room${roomId}`;
            ...
            <div class="material-tabs-content" id="materialTabsContent-${itemId}-${productId}-room${roomId}">
                ${materialCategories.map((category, index) => `
                    <div class="material-tab-content ${index === 0 ? 'active' : ''}" 
                        ...
                        ${createMaterialContentWithLabels(
                            product, 
                            variant, 
                            category, 
                            roomId, 
                            activeMaterials, 
                            productInstanceId,
                            {} // Empty existingData for order add page
                        )}
                    </div>
                `).join('')}
            </div>
