                     ...
                     <div class="compact-details-with-image">
                        ...
                        <div class="compact-details-fields">
                              <div class="compact-detail-group">
                                 <label>Width (m)</label>
                                 <input type="number" class="form-control dimension-width" 
                                    placeholder="0.00" step="0.01" min="0" value="${width}" 
                                    data-variant="${variant.id}" ${getDisabledAttr(product.calculate_type, 'w')}>
                              </div>
                              <div class="compact-detail-group">
                                 <label>Length (m)</label>
                                 <input type="number" class="form-control dimension-length" 
                                    placeholder="0.00" step="0.01" min="0" value="${length}" 
                                    data-variant="${variant.id}" ${getDisabledAttr(product.calculate_type, 'l')}>
                              </div>
                              <div class="compact-detail-group">
                                 <label>Height (m)</label>
                                 <input type="number" class="form-control dimension-height" 
                                    placeholder="0.00" step="0.01" min="0" value="${height}"
...
<script>
    ...
    // Function to add product to room that returns a Promise
            ...
            $.post(ajax_url + '/api', {
                ...
                try {
                    ...
                    if (response.status === 'success') {
                        ...
                        product.existing_data = productData.product_data;

                        console.log('Adding product tab with context:', {
                            roomNumber: roomNumber,
                            productId: product.product_id
                        });

                        // Add product tab with explicit room number
                        addProductTab(roomNumber, product, productData);

                        // Wait for the product content to load and calculations to complete
                        setTimeout(() => {
                            // Calculate product total
                            calculateProductTotal(product.product_id, roomNumber);

                            console.log(`Product ${product.product_id} fully loaded for room ${roomNumber}`);
                            resolve();
                        }, 2000); // Increased timeout for complex products
                    } else {
    ...
    // Enhanced function to populate product data with existing values that returns Promise
    function populateProductDataWithPromise(productId, roomId, existingData) {
        return new Promise((resolve) => {
            ...
            console.log('Populating product data:', existingData);

            // Populate basic fields
            if (existingData.quantity) {
                $productContent.find('.product-qty').val(existingData.quantity);
            }

            if (existingData.discount) {
                $productContent.find('.product-discount').val(existingData.discount);
            }

            if (existingData.width) {
                $productContent.find('.dimension-width').val(existingData.width);
            }

            if (existingData.length) {
                $productContent.find('.dimension-length').val(existingData.length);
            }

            if (existingData.height) {
                $productContent.find('.dimension-height').val(existingData.height);
            }

            // Populate curtain options
            if (existingData.curtain_options) {
                const options = existingData.curtain_options;
                if (options.opening_direction) {
                    $productContent.find('.opening-direction').val(options.opening_direction);
                }
                if (options.open_with) {
                    $productContent.find('.open-with').val(options.open_with);
                }
                if (options.installation_needed) {
                    $productContent.find('.curtain-installation-needed-checkbox').prop('checked', true);
                    updateCurtainInstallationPrice($productContent.find('.curtain-installation-needed-checkbox'));
                }
            }

            // Use setTimeout to ensure DOM is ready for complex operations
            setTimeout(() => {
                // Populate variants if any
                if (existingData.variants) {
                    existingData.variants.forEach(variant => {
                        if (variant.active) {
                            $(`#variant-${productId}-${variant.variant_id}-room${roomId} .product-qty`).val(variant.quantity);
                            $(`#variant-${productId}-${variant.variant_id}-room${roomId} .product-discount`).val(variant.discount);
                            $(`#variant-${productId}-${variant.variant_id}-room${roomId} .dimension-width`).val(variant.width);
                            $(`#variant-${productId}-${variant.variant_id}-room${roomId} .dimension-length`).val(variant.length);
                            $(`#variant-${productId}-${variant.variant_id}-room${roomId} .dimension-height`).val(variant.height);
                        }
                    });
                }

                // Populate materials with delay
                setTimeout(() => {
                    if (existingData.materials) {
                        populateMaterialsData(productId, roomId, existingData.materials);
                    }

                    // Populate surcharges
                    if (existingData.surcharges) {
                        populateSurchargesData(productId, roomId, existingData.surcharges);
                    }

                    // Populate accessories with more delay
                    setTimeout(() => {
                        if (existingData.accessories) {
                            populateAccessoriesData(productId, roomId, existingData.accessories);
                        }

                        // Populate items for fitout products with even more delay
                        setTimeout(() => {
                            if (existingData.items) {
                                populateItemsData(productId, roomId, existingData.items);
                            }

                            // Final calculation after all data is populated
                            setTimeout(() => {
                                calculateProductTotal(productId, roomId);
                                console.log(`Product ${productId} data fully populated for room ${roomId}`);
                                resolve();
                            }, 1000);
    ...
    function loadProductContentWithPromise(contentId, product, roomId, existingData = null) {
        return new Promise((resolve) => {
            ...
            // Check if we have existing data to pre-populate
            if (existingData) {
            ...
            if (product.has_variant && product.available_in === 'set') {
                $.post(ajax_url + '/api', {
                    ...
                    if (response.status === 'success') {
                        ...
                        if (existingData) {
                            populateProductDataWithPromise(product.product_id, roomId, existingData)
                                .then(() => {
                                    resolve();
                                });
                        } else {
                        ...
                        resolve(); // Resolve even if variants fail
                    }
                });
    ...
    // Function to populate materials data
    function populateMaterialsData(productId, roomId, materials) {
        materials.forEach(material => {
            const selector = `.material-type-select[data-category="${material.category}"][data-ref-label="${material.ref_label}"]`;
            $(`#product-${productId}-room${roomId} ${selector}`).val(material.material_id).trigger('change');

            const areaWeightSelector = `.area-weight[data-category="${material.category}"][data-ref-label="${material.ref_label}"]`;
            $(`#product-${productId}-room${roomId} ${areaWeightSelector}`).val(material.area_weight);
        });
    }
    ...
    function populateSurchargesData(productId, roomId, surcharges) {
        surcharges.forEach(surcharge => {
            if (surcharge.applied) {
                const checkbox = $(`#product-${productId}-room${roomId} .surcharge-checkbox[data-surcharge-name="${surcharge.name}"]`);
                if (checkbox.length) {
    ...
    function populateAccessoriesData(productId, roomId, accessories) {
        accessories.forEach(accessory => {
            // Find or create accessory tab
            const $tabsContainer = $(`#product-${productId}-room${roomId} .accessory-tabs-container`);
            const $emptyState = $tabsContainer.find('.empty-accessory-tabs');
            ...
            // Add accessory tab and content
            const tabId = `accessory-${accessory.accessory_id}-${productId}-room${roomId}`;
            const $tab = $(`
            <div class="accessory-tab" data-accessory-id="${accessory.accessory_id}" id="${tabId}-tab">
                <span class="accessory-tab-name">${accessory.name}</span>
                <div class="accessory-tab-close" title="Remove accessory">
                    <i class="fa fa-times"></i>
                </div>
        ...
        `);
            ...
            // Populate accessory content
            const $accessoryContent = $(`#${tabId}`);
            if ($accessoryContent.length) {
                $accessoryContent.find('.accessory-options-select').val(accessory.selected_option).trigger('change');
            }
        });
    }

    // Function to populate existing items data
    function populateItemsData(productId, roomId, items) {
        items.forEach(item => {
            // This will be handled by the main data loading process
...
<script>
    ...
    function collectProductDataForUpdate(productId, roomId, productType, availableIn, detailId = null) {
        ...
        // Collect dimensions for detail_attr
        const width = parseFloat($productContent.find('.dimension-width').val()) || 0;
        const length = parseFloat($productContent.find('.dimension-length').val()) || 0;
        const height = parseFloat($productContent.find('.dimension-height').val()) || 0;
        ...
        $(`#product-${productId}-room${roomId} .items-tab`).each(function() {
            ...
            if ($itemContent.length) {
                const itemData = {
                    ...
                    standart_price:parseFloat($itemContent.find('.standart_price').val()) || 0,
    ...
    function collectCurtainSetData($container, productId, variantId, roomId) {
        ...
        // Collect accessories
        $container.find('.accessory-tab').each(function() {
            const $accessoryTab = $(this);
            const accessoryId = $accessoryTab.data('accessory-id');
            const accessoryType = $accessoryTab.find('.accessory-tab-name').text();
            const $accessoryContent = $(`#accessory-${accessoryId}-${productId}${variantId ? '-' + variantId : ''}-room${roomId}`);
    ...
    function collectVariantDataForUpdate(productId, variantId, roomId, productType) {
        ...
        const variantData = {
            ...
            standart_length: parseFloat($variant.find('.dimension-length').data('standart_width')) || 0,
            standart_price:parseFloat($variant.find('.standart_price').val()) || 0,
