...
<script>
                            ...
                            $.post(ajax_url + '/api', {
                                ...
                                if (response.status === 'success') {
                                    ...
                                    if (selectedItem) {
                                        ...
                                        setTimeout(() => {
                                            calculateProductTotal(productInstanceId, roomId, 'fitout');
                                            resolve();
                                        }, 1000);
                                    } else {
    ...
    function addProductTab(roomId, product, existingData = null) {
        ...
        const $content = $(`
            <div class="product-content" id="${contentId}" style="display: none;">
    ...
    function applyProductFilters(qualificationIds, action = null) {
        action = action || '';
        ...
        if (currentCountProduct === 0 || search_text !== '' || action !== '') {
    ...
    $(document).on('click', '#loadMoreProductsBtn', function() {
        const qualifications = $('#productSelectModal').data('qualification');
        if (qualifications) {
            applyProductFilters(qualifications, 'load_more');
        }
    });
...
<script>
    ...
    function createMaterialGroupInputs(product, variant, category, refLabel, materials, roomId, variantId, categoryData, existingMaterials, existingCurtainData, index = 0) {
            ...
            for (const material of categoryMaterials) {
                if (material[0] === refLabel) {
                    existingMaterial = {
                        ...
                        replacement_image: material[2] && material[2] != '' ? (availableMaterials.find(mat => mat.material_id == material[2])?.material_img || '') : '',
                        replacement_material_name: material[2] && material[2] != '' ? (availableMaterials.find(mat => mat.material_id == material[2])?.material_name || '') : '',
        ...
        const fabricLength = existingCurtainData && existingCurtainData[index + 1] && existingCurtainData[index + 1].curtain ? existingCurtainData[index + 1].curtain.length : '';
        const fabricHeight = existingCurtainData && existingCurtainData[index + 1] && existingCurtainData[index + 1].curtain ? existingCurtainData[index + 1].curtain.height : '';
            ...
            <div class="material-group" data-ref-label="${refLabel}">
                ...
                <div class="material-grid">
                    ...
                    <div class="material-inputs-compact-replacement">
                        ...
                        ${currentMaterial && currentMaterial.replacement_image && currentMaterial.replacement_image !='' ? 
                                `<img src="<?= URL ?>/uploads/material/${currentMaterial.replacement_image}" alt="${currentMaterial.replacement_material_name}" style="width:100%;height:100%;object-fit:cover;">` : 
                                `<i class="fa fa-image"></i>`
                        }
                        </div>
                        <div class="material-compact-fields">
                            <div class="material-input">
                                    ...
                                    ${availableMaterials.map(material => `
                                        <option value="${material.material_id}" 
                                            ${currentMaterial && currentMaterial.replacement == material.material_id ? 'selected' : ''}
                                            data-price="${material.material_price || 0}"
                                            data-image="${material.material_img || ''}">
                                            ${material.material_name}
                                        </option>
    ...
    function createPillowSubcategoriesForVariant(product, variant, roomId, activeMaterials, existingMaterials, productInstanceId, index = 0) {
        const variantId = product.available_in === 'size' ? variant.id : variant.product_id;
        ...
        return `
            <div class="pillow-subcategories-section">
                <div class="pillow-subcategories-tabs" id="pillowTabs-${productInstanceId}-${variantId}-room${roomId}">
                    ${subcategories.map((subcat, index) => `
                        <button class="pillow-subcategory-tab ${index === 0 ? 'active' : ''}" 
                            data-subcategory="${subcat.id}" data-product="${productInstanceId}" data-variant="${variantId}">
                ...
                <div class="pillow-subcategories-content" id="pillowContent-${productInstanceId}-${variantId}-room${roomId}">
                    ${subcategories.map((subcat, index) => `
                        <div class="pillow-subcategory-content ${index === 0 ? 'active' : ''}" 
                        id="pillowSubcategory-${productInstanceId}-${variantId}-room${roomId}-${subcat.id}">
    ...
    function createPillowMaterialGroup(product, variant, subcat, refLabel, materials, allMaterials, roomId, variantId, existingMaterials, index = 0) {
        ...
        if (existingMaterials && existingMaterials.main && existingMaterials.main[index + 1] && existingMaterials.main[index + 1]['pillow']) {
            ...
            for (const material of pillowMaterials) {
                if (material[0] === refLabel) {
                    let replacement_key = `${subcat.id}_replacement`;
                    existingMaterial = {
                        refLabel: material[0],
                        ...
                        replacement: material[1][replacement_key] || '',
                        replacement_image: material[1][replacement_key] && material[1][replacement_key] != '' ? (availableMaterials.find(mat => mat.material_id == material[1][replacement_key])?.material_img || '') : '',
                        replacement_material_name: material[1][replacement_key] && material[1][replacement_key] != '' ? (availableMaterials.find(mat => mat.material_id == material[1][replacement_key])?.material_name || '') : '',
        ...
        return `
            <div class="pillow-material-group" data-ref-label="${refLabel}">
                ...
                <div class="material-grid">
                    ...
                    <div class="pillow-material-inputs-compact-replacement">
                    <div class="pillow-material-compact-image-replacement">
                        ${currentMaterial && currentMaterial.replacement_image && currentMaterial.replacement_image !='' ? 
                                `<img src="<?= URL ?>/uploads/material/${currentMaterial.replacement_image}" alt="${currentMaterial.replacement_material_name}" style="width:100%;height:100%;object-fit:cover;">` : 
                                `<i class="fa fa-image"></i>`
                        }
    ...
    function createVariantContentForSize(product, variant, roomId, existingData, index, productInstanceId) {
               ...
               <div class="product-variants-content" id="variants-content-${productId}-room${roomId}">
                  ${variants.map((variant, index) => `
                     <div class="product-variant-content ${variant.id === bed_dim ? 'active' : ''}" 
                        ...
                        ${createVariantContentForSize(product, variant, roomId, existingData, index, productInstanceId)}
                     </div>
                  `).join('')}
               </div>
            </div>
    ...
    function loadSimpleProductContent($content, product, variant, roomId, existingData, productInstanceId, index = 0) {
        ...
        setTimeout(() => {
            ...
            setupPillowSubcategoryTabs(product, variant, roomId, productInstanceId);
        }, 100);
    }
    ...
    function setupCurtainProductHandlers(product, roomId, productInstanceId) {
        ...
        setTimeout(() => {
            ...
            setupPillowSubcategoryTabs(product, product, roomId, productInstanceId);
        }, 100);
    }
    ...
    function createMaterialSection(product, variant, roomId, existingData, productInstanceId, index = 0) {
                  ...
                  <div class="material-tabs-content" id="materialTabsContent-${productInstanceId}-${variantId}-room${roomId}">
                     ${materialCategories.map((category, index) => `
                        <div class="material-tab-content ${index === 0 ? 'active' : ''}" 
                           ...
                           ${category.id === 'pillow' ? 
                                 createPillowSubcategoriesForVariant(product, variant, roomId, activeMaterials, existingMaterials, productInstanceId, index) : 
    ...
    function setupPillowSubcategoryTabs(product, variant, roomId, productInstanceId) {
    ...
    $(document).on('input change', '.product-content .area-weight, .product-content .curtain-fabric-length, .product-content .curtain-fabric-height, .product-content .material-type-select, .product-content .pillow-quantity, .product-content .pillow-length, .product-content .pillow-width', function() {
        const $productContent = $(this).closest('.product-content');
        if ($productContent.length) {
            const productId = $productContent.attr('id').replace('product-', '').replace(/-room\d+$/, '');
            const roomId = $productContent.attr('id').match(/room(\d+)/)[1];
...
<script>
    ...
    function collectMaterialsDataNewLayout(productId, roomId, variantId = null, itemId = null) {
        const materials = {
            main: {}
        };
        ...
        if (itemId) {
            ...
            prefix = `${productId}-${extractOriginalProductId(String(productId))}`;
        }
        console.log('Collecting Material Data:', {
            prifix: prefix,
        });

        const $container = $(`#materialTabsContent-${prefix}-room${roomId}`);
        ...
        $container.find('.material-tab-content').each(function() {
            ...
            $materialTab.find('.pillow-material-group').each(function() {
                ...
                const faceMaterial = $pillowGroup.find('.material-type-select[data-subcategory="front"]').val();
                const backMaterial = $pillowGroup.find('.material-type-select[data-subcategory="back"]').val();
                const pipingMaterial = $pillowGroup.find('.material-type-select[data-subcategory="pipping"]').val();
                const defaultMaterial = $pillowGroup.find('.material-type-select[data-subcategory="default"]').val();
