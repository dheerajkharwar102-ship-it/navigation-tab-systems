   ...
   jQuery(function($) {
         ...
         variants.forEach(variant => {
            ...
            setTimeout(() => {
               ...
               if ($materialTabs.length) {
                  ...
                  // Setup material calculations for each category
                  const activeMaterials = variant.active_materials || product.active_materials || {};
                  Object.keys(activeMaterials).forEach(category => {
                     if (category !== 'pillow') {
                        const categoryData = activeMaterials[category];
                        if (categoryData.materialGroups) {
                           Object.keys(categoryData.materialGroups).forEach(refLabel => {
                              setupMaterialCalculations(productId, variantId, roomId, category, refLabel);
                           });
                        } else {
                           setupMaterialCalculations(productId, variantId, roomId, category);
                        }
                     }
                  });
               }
    ...
    `);
      ...
      function loadSimpleProductContent($content, product, roomId) {
         ...
         setTimeout(() => {
            setupMaterialTabsForProduct(product, mockVariant, roomId);
            setupPillowSubcategoryTabs(product, mockVariant, roomId);
         }, 100);
      }
      ...
      function setupMaterialCalculations(productId, variantId, roomId, category, refLabel = '') {
         ...
         // Calculate total when measurement or material changes
         $(document).on('input change', selector, function() {
            ...
            const totalPrice = measurement * unitPrice;
