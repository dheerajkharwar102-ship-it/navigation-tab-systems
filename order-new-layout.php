...
if (isset($_POST['get_main_categories'])) {
    try {
        ...
        $menus = $pa->getWebMenus('*', null, [['deleted', '=', 0]]);

        if (empty($menus)) {
            echo json_response('success', 'No menus found', []);
            die;
        }

        // Prepare menu IDs for batch processing
        $menuIds = array_column($menus, 'id');

        // Get all attributes for these menus in one query (if supported by your ORM)
        // If not, at least prepare the structure
        $validMenus = [];

        foreach ($menus as $menu) {
            // Get attributes for this menu
            $where = [
                ['online_category', '=', $menu->id],
                ['attr_status', '=', '1']
            ];

            $attributes = $pa->getProductAttributes('*', ['attr_name', 'ASC'], $where);
            ...
            // Collect attribute IDs for batch product fetching
            $attrIds = array_column($attributes, 'attr_id');

            // Get products for all attributes at once (if possible)
            $whereProducts = [
                ['product_status', '=', '1']
            ];
            
            $inProducts = ['attr_id', $attrIds];

            $products = $p->getProducts('team_name,product_code,attr_id', null, $whereProducts, null, null, $inProducts);

            // if(empty($products)){
            //     continue;
            // }

            // Group products by attribute_id for easier processing
            $productsByAttr = [];
            foreach ($products as $product) {
                $productsByAttr[$product->attr_id][] = $product;
            }

            // Build attribute and product names
            $attrNames = [];
            $productNames = [];

            foreach ($attributes as $attr) {
                $attrNames[] = $attr->attr_name;

                // Get products for this specific attribute
                $attrProducts = $productsByAttr[$attr->attr_id] ?? [];

                foreach ($attrProducts as $product) {
                    $productNames[] = $product->team_name;
                    $productNames[] = $product->product_code;
                }
            }

            // Only add to result if we have attributes
            if (!empty($attrNames)) {
                $menu->attr_names = implode('-', $attrNames);
                $menu->product_names = implode('-', $productNames);
                $validMenus[] = $menu;
            }
        }

        echo json_response('success', 'OK', $menus);
    } catch (Exception $e) {
...
if (isset($_POST['get_qualifications'])) {
    ...
    if (!empty($search_text)) {
        $where[] = ['attr_name', 'LIKE', '%' . $search_text . '%'];
    }
    ...
    $get_attrs = $pa->getProductAttributesUnique('*', ['attr_id', 'ASC'], $where, null, true);
    $valid_attrs = [];
    foreach ($get_attrs as $attr) {
        ...
        // if (!isset($get_products) || count($get_products) < 1) {
        //     continue;
        // }
...
if (isset($_POST['get_products'])) {
    ...
    $is_fitout_type = isset($_POST['is_fitout_type']) ? strtolower(trim($_POST['is_fitout_type'])) : '';
